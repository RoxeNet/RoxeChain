...
The `http_plugin` is a core plugin supported by both `nodroxe` and `kroxed`. The plugin is required to enable any RPC API functionality provided by a `nodroxe` or `kroxed` instance.
...
```sh
# command-line
nodroxe ... --plugin roxe::http_plugin [options]
 (or)
kroxed ... --plugin roxe::http_plugin [options]
```
