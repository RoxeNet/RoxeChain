...
peer validation for similar hardware/roxe-version/config will be <= m
...
#       * image will be rendered by gatsby-remark-graphviz plugin in roxe docs.
...
Running wasm-runtime=roxe-vm-jit roxe-vm-oc-enable on relay node will reduce the validation time.
