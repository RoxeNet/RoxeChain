---
content_title: Nodroxe Plugins
---

## Overview

Plugins extend the core functionality implemented in `nodroxe`. Some plugins are mandatory, such as `chain_plugin`, `net_plugin`, and `producer_plugin`, which reflect the modular design of `nodroxe`. The other plugins are optional as they provide nice to have features, but non-essential for the nodes operation.
...
[[info | Nodroxe is modular]]
| Plugins add incremental functionality to `nodroxe`. Unlike runtime plugins, `nodroxe` plugins are built at compile-time.
