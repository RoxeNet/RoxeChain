...
The `chain_plugin` is a core plugin required to process and aggregate chain data on an ROXE node.
...
```console
Config Options for roxe::chain_plugin:
  ...
  --wasm-runtime roxe-vm|roxe-vm-jit      Override default WASM runtime (wabt)
  --roxe-vm-oc-enable                    Enable optimized compilation in WASM
