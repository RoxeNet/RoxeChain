---
content_title: Nodroxe Logging
---

Logging for `nodroxe` is controlled by the `logging.json` file. CLI options can be passed to `nodroxe` to [setup `logging.json`](00_setup-logging.json.md). The logging configuration file can be used to define [appenders](#appenders) and tie them to [loggers](#loggers) and [logging levels](01_logging-levels.md).

## Appenders

The logging library built into ROXE supports two appender types:
...
The logging library built into ROXE currently supports five loggers:
