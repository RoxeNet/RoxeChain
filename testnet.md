...
Before getting into the details of the actc testnet, lets clarify some terms. In this document I use the terms "host" and "machine" fairly interchangeably. A host generally boils down to a single IP address, although in practice it could have more.

The next term is "node." A node is an instance of the actcd executable configured to serve as 0 or more producers. There is not a one-to-one mapping between nodes and hosts, a host may serve more than one node, but one node cannot span more than one host.
...
Finally there is the idea of distributed networks that involve remote hosts. These may be hosts on which you may not have direct access for starting and stopping actcd instances, but with whom you may wish to collaborate for setting up a decentralized testnet.
...
Note that the Launcher will not push instances of actcd to the remote hosts, you must prepare the various test network hosts separately.
...
#### Star network
![](https://github.com/actc/actc/raw/master/star.png)
...
#### Mesh network
![](https://github.com/actc/actc/raw/master/mesh.png)
...
To address the complexity implied by distributing multiple actcd nodes across a LAN or a wider network, the launcher application was created.

Based on a handful of command line arguments the Launcher is able to compose per-node configuration files, distribute these files securely amongst the peer hosts, then start up the multiple instances of actcd.
...
launcher command line arguments:
  ...
  -d [ --delay ] arg (=0)               number of seconds to wait before starting the next node. Used to simulate a person keying in a series of individual actcd startup command lines.
  ...
  -o [ --output ] arg                   save a copy of the generated topology 
                                        in this file
  --skip-signature                      actcD does not require transaction
                                        signatures.
...
In this mode, the launcher does not activate any actcd instances, it produces a file of the given filename. This file is a JSON formatted template that provides an easy means of
...
```
  "nodes": [[
      "testnet_0",{
        ...
        "ssh_identity": "",
        "ssh_args": "",
        "actc_root_dir": "/home/phil/blockchain/actc",
        "data_dir": "tn_data_0",
        "hostname": "remoteserv",
        ...
        "filesize": 8192,
        "keys": [{
            "public_key": "actc6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV",
...
| nodes         | a collection of descriptors defining the actcd instances used to assemble this testnet. The names used as keys in this collection are also aliases used within as placeholders for peer nodes.
...
| actc_root_dir       | specifies the directory into which all actcd artifacts are based. This is required for any hosts that are not the local host.
...
It is also necessary to provision the server by at least copying the actcd executable, and the genesis.json files to their appropriate locations relative to some named actc root directory. For example, I defined the actc root to be `/home/phil/blockchain/actc`. When run, the launcher will run through a variety of shell commands using ssh and finally using scp to copy a config.ini file to the appropriate data directory on the remote.
...
| :------------ | :----------
| config.ini    | The actcd configuration file.
| actcd.pid      | The process ID of the running actcd instance.
| blockchain/*  | The blockchain backing store
| blocks/*      | The blockchain log store
| stderr.txt    | The cerr output from actcd.
| stdout.txt    | The cout output from actcd.
