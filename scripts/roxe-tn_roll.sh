#!/bin/bash
#
# roxe-tn_roll is used to have all of the instances of the ROXE daemon on a host brought down
# so that the underlying executable image file (the "text file") can be replaced. Then
# all instances are restarted.
# usage: roxe-tn_roll.sh [arglist]
...
# the command line.
#
# In most cases, simply running ./roxe-tn_roll.sh is sufficient.
#

if [ -z "$ROXE_HOME" ]; then
    echo ROXE_HOME not set - $0 unable to proceed.
    exit -1
fi
...
prog=""
RD=""
for p in roxed roxed nodroxe; do
    prog=$p
    RD=bin
    if [ -f $RD/$prog ]; then
        break;
...
if [ \( -z "$prog" \) -o \( -z "$RD" \) ]; then
    echo unable to locate binary for roxed or roxed or nodroxe
    exit 1
fi
...
if [ $DD = "all" ]; then
    for ROXE_RESTART_DATA_DIR in `ls -d var/lib/node_??`; do
        bash $ROXE_HOME/scripts/roxe-tn_up.sh "$*"
    done
else
    bash $ROXE_HOME/scripts/roxe-tn_up.sh "$*"
fi
unset ROXE_RESTART_DATA_DIR
