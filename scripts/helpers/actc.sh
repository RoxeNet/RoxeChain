...
if [[ -z "${NAME}" ]]; then
    ...
    else echo " ${COLOR_RED}- ACTC is not supported for your Architecture!${COLOR_NC}" && exit 1
    fi
fi

# Setup yum and apt variables
if [[ $NAME =~ "Amazon Linux" ]] || [[ $NAME == "CentOS Linux" ]]; then
    if ! YUM=$( command -v yum 2>/dev/null ); then echo "${COLOR_RED}YUM must be installed to compile ACTC${COLOR_NC}" && exit 1; fi
elif [[ $NAME == "Ubuntu" ]]; then
    if ! APTGET=$( command -v apt-get 2>/dev/null ); then echo "${COLOR_RED}APT-GET must be installed to compile ACTC${COLOR_NC}" && exit 1; fi
fi
...
function setup() {
    if $VERBOSE; then
        ...
        echo "ACTC_INSTALL_DIR: ${ACTC_INSTALL_DIR}"
...
function ensure-which() {
  if ! which ls &>/dev/null; then
    while true; do
      [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}ACTC compiler checks require the 'which' package: Would you like for us to install it? (y/n)?${COLOR_NC}" && read -p " " PROCEED
      echo ""
      case $PROCEED in
...
function install-directory-prompt() {
    if [[ -z $INSTALL_LOCATION ]]; then
        echo "No installation location was specified. Please provide the location where ACTC is installed."
        while true; do
            [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}Do you wish to use the default location? ${ACTC_INSTALL_DIR}? (y/n)${COLOR_NC}" && read -p " " PROCEED
            echo ""
            case $PROCEED in
                "" )
                echo "What would you like to do?";;
                ...
                break;;
                1 | false | [Nn]* )
                printf "Enter the desired installation location." && read -p " " ACTC_INSTALL_DIR;
                export ACTC_INSTALL_DIR;
                break;;
                * ) echo "Please type 'y' for yes or 'n' for no.";;
            esac
        done
    else
        # Support relative paths : https://github.com/ACTC/actc/issues/7560
        [[ ! $INSTALL_LOCATION =~ ^\/ ]] && export INSTALL_LOCATION="${CURRENT_WORKING_DIR}/$INSTALL_LOCATION"
        export ACTC_INSTALL_DIR="$INSTALL_LOCATION"
    fi
    . ./scripts/.build_vars
    echo "ACTC will be installed to: ${ACTC_INSTALL_DIR}"
}

function previous-install-prompt() {
  if [[ -d $ACTC_INSTALL_DIR ]]; then
    echo "ACTC has already been installed into ${ACTC_INSTALL_DIR}... It's suggested that you actc_uninstall.sh before re-running this script."
    while true; do
...
function resources() {
    echo "${COLOR_CYAN}ACTC website:${COLOR_NC} https://actc.io"
    echo "${COLOR_CYAN}ACTC Telegram channel:${COLOR_NC} https://t.me/ACTCProject"
    echo "${COLOR_CYAN}ACTC resources:${COLOR_NC} https://actc.io/resources/"
    echo "${COLOR_CYAN}ACTC Stack Exchange:${COLOR_NC} https://actc.stackexchange.com"
}

function print_supported_linux_distros_and_exit() {
   echo "On Linux the ACTC build script only supports Amazon, Centos, and Ubuntu."
...
function ensure-compiler() {
    ...
    if $PIN_COMPILER || [[ -f $CLANG_ROOT/bin/clang++ ]]; then
        ...
        if [[ $CXX =~ "clang" ]] || [[ $COMPILER_TYPE =~ "clang" ]]; then
            ...
            if [[ $NO_CPP17 == false ]]; then # https://github.com/ACTC/actc/issues/7402
                while true; do
                    echo "${COLOR_YELLOW}WARNING: Your GCC compiler ($CXX) is less performant than clang (https://github.com/ACTC/actc/issues/7402). We suggest running the build script with -P or install your own clang and try again.${COLOR_NC}"
                    [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}Do you wish to proceed anyway? (y/n)?${COLOR_NC}" && read -p " " PROCEED
                    case $PROCEED in
                        "" ) echo "What would you like to do?";;
                        0 | true | [Yy]* ) break;;
...
function ensure-cmake() {
    ...
    if [[ ! -e "${CMAKE}" ]]; then
        ...
        && ./bootstrap --prefix=${ACTC_INSTALL_DIR} \
        && make -j${JOBS} \
        && make install \
        && cd .. \
