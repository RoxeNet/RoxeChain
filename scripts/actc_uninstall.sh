...
function usage() {
  ...
  -i DIR      Directory where actc is installed)
...
if [[ $NONINTERACTIVE == false ]] && $FULL; then
   while true; do
      read -p "By specifying -f, removal of the actc data directory will require a resync of data which can take days. Do you wish to proceed? (y/n) " PROCEED
      case $PROCEED in
      ...
      case $PROCEED in
         ...
         0 | true | [Yy]* )
            ...
            if $FULL; then
               ...
               [[ $ARCH == "Darwin" ]] && INSTALL_PATHS+=("${HOME}/Library/Application\ Support/actc")
               [[ $ARCH != "Darwin" ]] && INSTALL_PATHS+=("${HOME}/.local/share/actc")
            fi
            # Version < 1.8.0; Before we started using ~/actc/1.8.x
...
echo "[Removing ACTC Dependencies]"
if [[ $ARCH == "Darwin" ]]; then
   for package in $(cat scripts/actc_build_darwin_deps | cut -d, -f1 2>/dev/null); do
      while true; do
