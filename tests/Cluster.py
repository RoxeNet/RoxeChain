...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="dccio-ignition-wd/bootlog.txt"
    __configDir="etc/dccio/"
    __dataDir="var/lib/"
    __fileDivider="================================================================="

    # pylint: disable=too-many-arguments
    # walletd [True|False] Is kdccd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="dcctest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet kdccd running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: dcc server host
        port: dcc server port
        walletHost: dcc wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures dcc mongo plugin
        ...
        self.dccioAccount=self.defProducerAccounts["dccio"]= Account("dccio")
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, extraNoddccArgs=None, useBiosBootFile=True, specificExtraNoddccArgs=None):
        """Launch cluster.
        ...
        extraNoddccArgs: string of arguments to pass through to each nodoes instance (via --noddcc flag on launcher)
        ...
        specificExtraNoddccArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-noddcc flags on launcher), example: { "5" : "--plugin dccio::test_control_api_plugin" }
        """
        assert(isinstance(topo, str))

        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.dccServerName))
            return True
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on noddcc default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        cmd="%s -p %s -n %s -d %s -i %s -f --p2p-plugin %s %s" % (
            Utils.dccLauncherPath, pnodes, totalNodes, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            p2pPlugin, producerFlag)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        noddccArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on * --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            noddccArgs += " --plugin dccio::wallet_api_plugin"
        if self.enableMongo:
            noddccArgs += " --plugin dccio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNoddccArgs is not None:
            assert(isinstance(extraNoddccArgs, str))
            noddccArgs += extraNoddccArgs
        if Utils.Debug:
            noddccArgs += " --contracts-console"

        if noddccArgs:
            cmdArr.append("--noddcc")
            cmdArr.append(noddccArgs)

        if specificExtraNoddccArgs is not None:
            assert(isinstance(specificExtraNoddccArgs, dict))
            for nodeNum,arg in specificExtraNoddccArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-noddcc")
                cmdArr.append(arg)
        ...
        if nodes is None or totalNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.dccServerName, totalNodes, len(nodes)))
            return False
        ...
        self.dccioAccount=self.defProducerAccounts["dccio"]
    ...
    def getClientVersion(verbose=False):
        ...
        try:
            cmd="%s version client" % (Utils.dccClientPath)
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key --to-console" % (Utils.dccClientPath)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on dcc server port %d" % (
        ...
        for i in range(0, count):
            ...
            for _ in range(0, count):
                #Utils.Print("nextdccIdx: %d, n: %d" % (nextdccIdx, n))
                nextdccIdx=(nextdccIdx + 1)%count
                if not self.nodes[nextdccIdx].killed:
                    #Utils.Print("nextdccIdx: %d" % (nextdccIdx))
            ...
            #Utils.Print("nextdccIdx: %d, count: %d" % (nextdccIdx, count))
            node=self.nodes[nextdccIdx]
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if Utils.Debug: Utils.Print("Validate funds on %s server port %d." %
                                        (Utils.dccServerName, node.port))

            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on dcc node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        initialBalances=self.nodes[0].getdccBalances([self.defproduceraAccount] + self.accounts)
        assert(initialBalances)
        assert(isinstance(initialBalances, dict))
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.dccioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse dccio private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(totalNodes, biosHost, biosPort, walletMgr, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by dccio-launcher."""
        ...
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down dcc cluster.")
            return None
        ...
        dccioName="dccio"
        dccioKeys=producerKeys[dccioName]
        dccioAccount=Account(dccioName)
        dccioAccount.ownerPrivateKey=dccioKeys["private"]
        dccioAccount.ownerPublicKey=dccioKeys["public"]
        dccioAccount.activePrivateKey=dccioKeys["private"]
        dccioAccount.activePublicKey=dccioKeys["public"]
        producerKeys.pop(dccioName)

        if not walletMgr.importKey(dccioAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (dccioName))
            return None

        initialFunds="1000000.0000 {0}".format(CORE_SYMBOL)
        Utils.Print("Transfer initial fund %s to individual accounts." % (initialFunds))
        trans=None
        contract="dccio.token"
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"dccio\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init transfer")
            opts="--permission dccio@active"
            if name != "dccio":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from dccio.token to %s." % (name))
                    return None
    ...
    def bootstrap(totalNodes, prodCount, totalProducers, biosHost, biosPort, walletMgr, onlyBios=False):
        ...
        trans=biosNode.publishContract(dccioAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, dccioAccount, 0)
            if trans is None:
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission dccio@active"
                    myTrans=biosNode.pushMessage("dccio", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("dccio", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but dccio).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "dccio"
            ret=Utils.waitForBool(lam)
            if not ret:
                Utils.Print("ERROR: Block production handover failed.")
                return None

        dccioTokenAccount=copy.deepcopy(dccioAccount)
        dccioTokenAccount.name="dccio.token"
        trans=biosNode.createAccount(dccioTokenAccount, dccioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (dccioTokenAccount.name))
            return None

        dccioRamAccount=copy.deepcopy(dccioAccount)
        dccioRamAccount.name="dccio.ram"
        trans=biosNode.createAccount(dccioRamAccount, dccioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (dccioRamAccount.name))
            return None

        dccioRamfeeAccount=copy.deepcopy(dccioAccount)
        dccioRamfeeAccount.name="dccio.ramfee"
        trans=biosNode.createAccount(dccioRamfeeAccount, dccioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (dccioRamfeeAccount.name))
            return None

        dccioStakeAccount=copy.deepcopy(dccioAccount)
        dccioStakeAccount.name="dccio.stake"
        trans=biosNode.createAccount(dccioStakeAccount, dccioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (dccioStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(dccioTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=dccioTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\",\"can_freeze\":\"0\",\"can_recall\":\"0\",\"can_whitelist\":\"0\"}" % (dccioTokenAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to dccio contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (dccioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to dccio contract.")
            return None
        ...
        Utils.Print("Verify dccio issue, Expected: %s" % (expectedAmount))
        actualAmount=biosNode.getAccountdccBalanceStr(dccioAccount.name)
        if expectedAmount != actualAmount:
            Utils.Print("ERROR: Issue verification failed. Excepted %s, actual: %s" %
                        (expectedAmount, actualAmount))
            return None

        contract="dccio.system"
        contractDir="contracts/%s" % (contract)
        wasmFile="%s.wasm" % (contract)
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (dccioAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (dccioAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (dccioTokenAccount.name, name))
                return None
    ...
    @staticmethod
    def pgrepdccServers(timeout=None):
        cmd=Utils.pgrepCmd(Utils.dccServerName)
    ...
    @staticmethod
    def pgrepdccServerPattern(nodeInstance):
        dataLocation=Cluster.__dataDir + Cluster.nodeExtensionToName(nodeInstance)
        return r"[\n]?(\d+) (.* --data-dir %s .*)\n" % (dataLocation)

    # Populates list of dccInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes, timeout=None):
        nodes=[]

        psOut=Cluster.pgrepdccServers(timeout)
        if psOut is None:
        ...
        for i in range(0, totalNodes):
            pattern=Cluster.pgrepdccServerPattern(i)
            m=re.search(pattern, psOut, re.MULTILINE)
            if m is None:
                Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.dccServerName, pattern))
                break
    ...
    def discoverBiosNodePid(self, timeout=None):
        psOut=Cluster.pgrepdccServers(timeout=timeout)
        pattern=Cluster.pgrepdccServerPattern("bios")
        Utils.Print("pattern={\n%s\n}, psOut=\n%s\n" % (pattern,psOut))
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.dccServerName, pattern))
    ...
    # Kills a percentange of dcc instances starting from the tail and update dccInstanceInfos state
    def killSomedccInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.dccServerName, killSignal))
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster noddcc instances. allInstances will kill all noddcc instances running on the system."""
        cmd="%s -k 9" % (Utils.dccLauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down dcc cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the dcc server
            cmd="pkill -9 %s" % (Utils.dccServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down dcc cluster.")
    ...
    def bounce(self, nodes, silent=True):
        """Bounces noddcc instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down noddcc instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
