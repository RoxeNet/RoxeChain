...
SCRIPT_LOCATION="scripts/roxe_build.sh"
TEST_LABEL="[roxe_build]"

###################################################################
# ON MAC YOU NEED TO FULLY UNINSTALL ROXE BEFORE THESE WILL PASS #
###################################################################
...
@test "${TEST_LABEL} > Testing Arguments & Options" {
    ...
    if [[ $NAME =~ "Amazon Linux" ]] || [[ $NAME == "CentOS Linux" ]]; then
        ...
        [[ ! -z $(echo "${output}" | grep "ROXE compiler checks require the 'which'") ]] || exit
    fi
    ...
    # lack of -i
    [[ ! -z $(echo "${output}" | grep "ROXE_INSTALL_DIR: ${HOME}/roxe/${ROXE_VERSION}") ]] || exit
    ## -o
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -o Debug -P"
    [[ ! -z $(echo "${output}" | grep "CMAKE_BUILD_TYPE: Debug") ]] || exit
    ## -s
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -s ROXE2 -P"
    [[ ! -z $(echo "${output}" | grep "CORE_SYMBOL_NAME: ROXE2") ]] || exit
    ## -b
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -b /test -P"
    [[ ! -z $(echo "${output}" | grep "BOOST_LOCATION: /test") ]] || exit
    ## -i
    run bash -c "printf \"y\ny\nn\nn\n\"| ./$SCRIPT_LOCATION -i /NEWPATH -P"
    [[ ! -z $(echo "${output}" | grep "ROXE_INSTALL_DIR: /NEWPATH") ]] || exit
    [[ ! -z $(echo "${output}" | grep "TEMP_DIR: ${HOME}/tmp") ]] || exit
        ...
        [[ ! -z $(echo "${output}" | grep "ROXE_INSTALL_DIR: $TEMP_DIR/NEWPATH") ]] || exit
