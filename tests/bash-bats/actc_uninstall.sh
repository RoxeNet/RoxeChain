...
SCRIPT_LOCATION="scripts/actc_uninstall.sh"
TEST_LABEL="[actc_uninstall]"
...
@test "${TEST_LABEL} > Testing user prompts" {
  ...
  [[ $output =~ " - ACTC Removal Complete" ]] || exit
  ## First no shows "Cancelled..."
  run bash -c "echo \"n\" | ./$SCRIPT_LOCATION"
  [[ "${output##*$'\n'}" =~ "Cancelled ACTC Removal!" ]] || exit
...
@test "${TEST_LABEL} > -i" {
  run ./$SCRIPT_LOCATION -y -i actctest
  [[ $output =~ .*/actctest ]] || exit
  ([[ ! $output =~ "Library/Application\ Support/actc" ]] && [[ ! $output =~ ".local/share/actc" ]]) || exit
  [[ ! $output =~ "ACTC Removal Complete" ]] || exit
}
