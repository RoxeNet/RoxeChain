...
export SCRIPT_LOCATION="scripts/roxe_build.sh"
export TEST_LABEL="[roxe_build_darwin]"
...
## Needed to load roxe_build_ files properly; it can be empty
@test "${TEST_LABEL} > General" {
    ...
    [[ ! -z $(echo "${output}" | grep "Starting ROXE Dependency Install") ]] || exit
    ...
    [[ ! -z $(echo "${output}" | grep "brew tap roxe/roxe") ]] || exit
    ...
    [[ ! -z $(echo "${output}" | grep "Starting ROXE Build") ]] || exit
    [[ ! -z $(echo "${output}" | grep " --with-iostreams --with-date_time") ]] || exit # BOOST
    [[ ! -z $(echo "${output}" | grep "ROXE has been successfully built") ]] || exit
}
