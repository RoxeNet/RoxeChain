...
SCRIPT_LOCATION="scripts/roxe_uninstall.sh"
TEST_LABEL="[roxe_uninstall]"
...
@test "${TEST_LABEL} > Testing user prompts" {
  ...
  [[ $output =~ " - ROXE Removal Complete" ]] || exit
  ## First no shows "Cancelled..."
  run bash -c "echo \"n\" | ./$SCRIPT_LOCATION"
  [[ "${output##*$'\n'}" =~ "Cancelled ROXE Removal!" ]] || exit
...
@test "${TEST_LABEL} > -i" {
  run ./$SCRIPT_LOCATION -y -i roxetest
  [[ $output =~ .*/roxetest ]] || exit
  ([[ ! $output =~ "Library/Application\ Support/roxe" ]] && [[ ! $output =~ ".local/share/roxe" ]]) || exit
  [[ ! $output =~ "ROXE Removal Complete" ]] || exit
}
