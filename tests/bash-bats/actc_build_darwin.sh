...
export SCRIPT_LOCATION="scripts/actc_build.sh"
export TEST_LABEL="[actc_build_darwin]"
...
## Needed to load actc_build_ files properly; it can be empty
@test "${TEST_LABEL} > General" {
    ...
    [[ ! -z $(echo "${output}" | grep "Starting ACTC Dependency Install") ]] || exit
    ...
    [[ ! -z $(echo "${output}" | grep "brew tap actc/actc") ]] || exit
    ...
    [[ ! -z $(echo "${output}" | grep "Starting ACTC Build") ]] || exit
    [[ ! -z $(echo "${output}" | grep " --with-iostreams --with-date_time") ]] || exit # BOOST
    [[ ! -z $(echo "${output}" | grep "ACTC has been successfully built") ]] || exit
}
