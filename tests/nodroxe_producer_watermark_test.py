...
###############################################################
# nodroxe_producer_watermark_test
...
def setProds(sharedProdKey):
    ...
    trans=cluster.biosNode.pushMessage("roxe", "setprods", setProdsStr, opts)
...
try:
    ...
    while tries > 0:
        ...
        info = node.getInfo()
        if node.infoValid and node.lastRetrievedHeadBlockProducer != "roxe":
            break
        time.sleep(1)
        tries = tries-1
    if tries == 0:
