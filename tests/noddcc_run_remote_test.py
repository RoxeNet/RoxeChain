...
###############################################################
# noddcc_run_remote_test
#  Tests remote capability of the noddcc_run_test. Test will setup cluster and pass nodes info to noddcc_run_test. E.g.
#  noddcc_run_remote_test.py -v --clean-run --dump-error-detail
###############################################################
...
pnodes=1
total_nodes=pnodes
actualTest="tests/noddcc_run_test.py"
...
try:
    ...
    Print("Starting up %s test: %s" % ("noddcc", actualTest))
    Print("cmd: %s\n" % (cmd))
    ...
    TestHelper.shutdown(cluster, None, testSuccessful, killdccInstances, False, False, killAll, dumpErrorDetails)
