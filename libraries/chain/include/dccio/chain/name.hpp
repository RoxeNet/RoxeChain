...
#define N(X) dccio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<dccio::chain::name> : private hash<uint64_t> {
      typedef dccio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const dccio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, dccio::chain::name& check);
} // fc


FC_REFLECT( dccio::chain::name, (value) )
