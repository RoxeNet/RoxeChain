...
namespace dccio { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(dccio);
const static uint64_t null_account_name      = N(dccio.null);
const static uint64_t producers_account_name = N(dccio.prods);
...
const static uint64_t dccio_auth_scope       = N(dccio.auth);
const static uint64_t dccio_all_scope        = N(dccio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t dccio_any_name = N(dccio.any);
const static uint64_t dccio_code_name = N(dccio.code);
...
const static dccio::chain::wasm_interface::vm_type default_wasm_runtime = dccio::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace dccio::chain::config

constexpr uint64_t dcc_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / dccio::chain::config::percent_100;
}

template<typename Number>
Number dcc_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + dccio::chain::config::percent_100 - dccio::chain::config::percent_1)  / dccio::chain::config::percent_100;
}
