...
#define N(X) roxe::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<roxe::chain::name> : private hash<uint64_t> {
      typedef roxe::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const roxe::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, roxe::chain::name& check);
} // fc


FC_REFLECT( roxe::chain::name, (value) )
