...
namespace roxe { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(roxe);
const static uint64_t null_account_name      = N(roxe.null);
const static uint64_t producers_account_name = N(roxe.prods);
...
const static uint64_t roxe_auth_scope       = N(roxe.auth);
const static uint64_t roxe_all_scope        = N(roxe.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t roxe_any_name = N(roxe.any);
const static uint64_t roxe_code_name = N(roxe.code);
...
const static roxe::chain::wasm_interface::vm_type default_wasm_runtime = roxe::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace roxe::chain::config

constexpr uint64_t ROXE_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / roxe::chain::config::percent_100;
}

template<typename Number>
Number ROXE_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + roxe::chain::config::percent_100 - roxe::chain::config::percent_1)  / roxe::chain::config::percent_100;
}
