...
#define N(X) actc::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<actc::chain::name> : private hash<uint64_t> {
      typedef actc::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const actc::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, actc::chain::name& check);
} // fc


FC_REFLECT( actc::chain::name, (value) )
