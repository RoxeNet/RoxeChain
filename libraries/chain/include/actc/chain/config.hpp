...
namespace actc { namespace chain { namespace config {
...
const static uint64_t actc_auth_scope       = N(actc.auth);
const static uint64_t actc_all_scope        = N(actc.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t actc_any_name = N(actc.any);
const static uint64_t actc_code_name = N(actc.code);
...
const static actc::chain::wasm_interface::vm_type default_wasm_runtime = actc::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace actc::chain::config

constexpr uint64_t ACTC_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / actc::chain::config::percent_100;
}

template<typename Number>
Number ACTC_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + actc::chain::config::percent_100 - actc::chain::config::percent_1)  / actc::chain::config::percent_100;
}
