...
void validate_authority_precondition( const apply_context& context, const authority& auth ) {
   for(const auto& a : auth.accounts) {
      ...
      if( a.permission.permission == config::dccio_code_name ) // virtual dccio.code permission does not really exist but is allowed
         continue;

      try {
         context.control.get_authorization_manager().get_permission({a.permission.actor, a.permission.permission});
      } catch( const permission_query_exception& ) {
         dcc_THROW( action_validate_exception,
 ...
 */
void apply_dccio_newaccount(apply_context& context) {
   auto create = context.act.data_as<newaccount>();
   try {
   context.require_authorization(create.creator);
//   context.require_write_lock( config::dccio_auth_scope );
   auto& authorization = context.control.get_mutable_authorization_manager();

   dcc_ASSERT( validate(create.owner), action_validate_exception, "Invalid owner authority");
   dcc_ASSERT( validate(create.active), action_validate_exception, "Invalid active authority");

   auto& db = context.db;

   auto name_str = name(create.name).to_string();

   dcc_ASSERT( !create.name.empty(), action_validate_exception, "account name cannot be empty" );
   dcc_ASSERT( name_str.size() <= 12, action_validate_exception, "account names can only be 12 chars long" );

   // Check if the creator is privileged
   const auto &creator = db.get<account_object, by_name>(create.creator);
   if( !creator.privileged ) {
      dcc_ASSERT( name_str.find( "dccio." ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with 'dccio.'" );
   }

   auto existing_account = db.find<account_object, by_name>(create.name);
   dcc_ASSERT(existing_account == nullptr, account_name_exists_exception,
...
void apply_dccio_setcode(apply_context& context) {
   ...
   dcc_ASSERT( act.vmtype == 0, invalid_contract_vm_type, "code should be 0" );
   dcc_ASSERT( act.vmversion == 0, invalid_contract_vm_version, "version should be 0" );
   ...
   dcc_ASSERT( account.code_version != code_id, set_exact_code, "contract is already running this version of code" );
...
void apply_dccio_setabi(apply_context& context) {
   auto& db  = context.db;
...
void apply_dccio_updateauth(apply_context& context) {
   ...
   dcc_ASSERT(!update.permission.empty(), action_validate_exception, "Cannot create authority with empty name");
   dcc_ASSERT( update.permission.to_string().find( "dccio." ) != 0, action_validate_exception,
               "Permission names that start with 'dccio.' are reserved" );
   dcc_ASSERT(update.permission != update.parent, action_validate_exception, "Cannot set an authority as its own parent");
   db.get<account_object, by_name>(update.account);
   dcc_ASSERT(validate(update.auth), action_validate_exception,
              "Invalid authority: ${auth}", ("auth", update.auth));
   if( update.permission == config::active_name )
      dcc_ASSERT(update.parent == config::owner_name, action_validate_exception, "Cannot change active authority's parent from owner", ("update.parent", update.parent) );
   if (update.permission == config::owner_name)
      dcc_ASSERT(update.parent.empty(), action_validate_exception, "Cannot change owner authority's parent");
   else
      dcc_ASSERT(!update.parent.empty(), action_validate_exception, "Only owner permission can have empty parent" );

   if( update.auth.waits.size() > 0 ) {
      auto max_delay = context.control.get_global_properties().configuration.max_transaction_delay;
      dcc_ASSERT( update.auth.waits.back().wait_sec <= max_delay, action_validate_exception,
   ...
   if( permission ) {
      dcc_ASSERT(parent_id == permission->parent, action_validate_exception,
...
void apply_dccio_deleteauth(apply_context& context) {
//   context.require_write_lock( config::dccio_auth_scope );
   ...
   dcc_ASSERT(remove.permission != config::active_name, action_validate_exception, "Cannot delete active authority");
   dcc_ASSERT(remove.permission != config::owner_name, action_validate_exception, "Cannot delete owner authority");
...
void apply_dccio_linkauth(apply_context& context) {
//   context.require_write_lock( config::dccio_auth_scope );

   auto requirement = context.act.data_as<linkauth>();
   try {
      dcc_ASSERT(!requirement.requirement.empty(), action_validate_exception, "Required permission cannot be empty");
      ...
      dcc_ASSERT(account != nullptr, account_query_exception,
      ...
      dcc_ASSERT(code != nullptr, account_query_exception,
                 "Failed to retrieve code for account: ${account}", ("account", requirement.code));
      if( requirement.requirement != config::dccio_any_name ) {
         const auto *permission = db.find<permission_object, by_name>(requirement.requirement);
         dcc_ASSERT(permission != nullptr, permission_query_exception,
      ...
      if( link ) {
         dcc_ASSERT(link->required_permission != requirement.requirement, action_validate_exception,
...
void apply_dccio_unlinkauth(apply_context& context) {
//   context.require_write_lock( config::dccio_auth_scope );
   ...
   auto link = db.find<permission_link_object, by_action_name>(link_key);
   dcc_ASSERT(link != nullptr, action_validate_exception, "Attempting to unlink authority, but no link found");
   context.add_ram_usage(
      link->account,
      -(int64_t)(config::billable_size_v<permission_link_object>)
   );
...
void apply_dccio_canceldelay(apply_context& context) {
