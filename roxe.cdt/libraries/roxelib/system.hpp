...
#warning "<roxelib/system.hpp> is deprecated use <roxe/system.hpp>"
namespace roxe {
   ...
   inline void check(bool pred, const char* msg, size_t n) {
      if (!pred) {
         roxe_assert_message(false, msg, n);
      }
   }
   ...
   inline void check(bool pred, const std::string& msg, size_t n) {
      if (!pred) {
         roxe_assert_message(false, msg.c_str(), n);
      }
   }
