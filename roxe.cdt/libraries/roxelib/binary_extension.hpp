...
#warning "<roxelib/binary_extension.hpp> is deprecated use <roxe/binary_extension.hpp>"
 ...
 namespace roxe {
   ...
   class binary_extension {
      public:
         ...
         constexpr T& value()& {
            if (!_has_value) {
               roxe_assert(false, "cannot get value of empty binary_extension");
            }
            return _get();
         }
          /** get the contained value */
         ...
         **/
         ...
         friend inline DataStream& operator<<(DataStream& ds, const roxe::binary_extension<T>& be) {
            ds << be.value_or();
            return ds;
         }
         ...
         /**
           ...
           */
         template<typename DataStream>
         friend inline DataStream& operator>>(DataStream& ds, roxe::binary_extension<T>& be) {
            if( ds.remaining() ) {
               T val;
               ds >> val;
               be.emplace(val);
