 ...
 */
#pragma once
#include <roxelib/types.h>

#warning "<roxelib/db.h> is deprecated use <roxe/db.h>. If you are using C++ the .h header files will be removed from inclusion entirely in v1.7.0"

/**
 *  @addtogroup database_c_api Database C API
 *  @ingroup c_api
 *  @brief Defines %C APIs for interfacing with the database.
 *  @details Database C API provides low level interface to ROXE database.
 *
...
/**
  ...
  *  roxe_assert(itr >= 0, "Alice cannot be removed since she was already not found in the table");
  *  db_remove_i64(itr);
  *  @endcode
  */
__attribute__((roxe_wasm_import))
...
/**
  ...
  *  roxe_assert(len <= 50, "buffer to small to store retrieved record");
...
/**
  ...
  *  roxe_assert(end_itr < -1, "Charlie was not the last entry in the table");
  *  @endcode
  */
__attribute__((roxe_wasm_import))
