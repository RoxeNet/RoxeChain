...
#warning "<roxelib/datastream.hpp> is deprecated use <roxe/datastream.hpp>"
   ...
   public:
      ...
      */
      inline bool read( char* d, size_t s ) {
        roxe::check( size_t(_end - _pos) >= (size_t)s, "read" );
        memcpy( d, _pos, s );
        _pos += s;
        return true;
      }
      ...
      */
      inline bool write( const char* d, size_t s ) {
        roxe::check( _end - _pos >= (int32_t)s, "write" );
...
void deserialize(datastream<Stream>& ds, std::variant<Ts...>& var, int i) {
   if constexpr (I < std::variant_size_v<std::variant<Ts...>>) {
      ...
      roxe::check(false, "invalid variant index");
   }
}
...
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const roxe::symbol sym) {
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, roxe::symbol& sym) {
  uint64_t raw = 0;
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const ::roxe::ignore_wrapper<T>& val) {
  ds << val.value;
  return ds;
}
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const ::roxe::ignore<T>& val) {
  return ds;
}
 ...
 */
template<typename Stream, typename T>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, ::roxe::ignore<T>) {
  return ds;
}
 ...
 *
 *  @brief Serialize an roxe::public_key
 *  @param ds - The stream to write
 *  @param pubkey - The value to serialize
 *  @tparam Stream - Type of datastream buffer
 *  @return datastream<Stream>& - Reference to the datastream
 */
template<typename Stream>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const roxe::public_key& pubkey) {
   ds << pubkey.type;
 ...
 *
 *  @brief Deserialize an roxe::public_key
 *  @param ds - The stream to read
 *  @param pubkey - The destination for deserialized value
 *  @tparam Stream - Type of datastream buffer
 *  @return datastream<Stream>& - Reference to the datastream
 */
template<typename Stream>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, roxe::public_key& pubkey) {
   ds >> pubkey.type;
 ...
 *
 *  @brief Serialize an roxe::signature
 *  @param ds - The stream to write
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator<<(datastream<Stream>& ds, const roxe::signature& sig) {
   ds << sig.type;
 ...
 *
 *  @brief Deserialize an roxe::signature
 *  @param ds - The stream to read
 ...
 */
template<typename Stream>
inline datastream<Stream>& operator>>(datastream<Stream>& ds, roxe::signature& sig) {
   ds >> sig.type;
...
DataStream& operator >> ( DataStream& ds, T (&v)[N] ) {
   unsigned_int s;
   ds >> s;
   roxe::check( N == s.value, "T[] size and unpacked size don't match");
   for( uint32_t i = 0; i < N; ++i )
      ds >> v[i];
   return ds;
}
