...
#warning "<roxe/native/intrinsics_def.hpp> is deprecated use <roxe/intrinsics_def.hpp>"
namespace roxe { namespace native {
   ...
   auto create_function(std::index_sequence<Is...>) {
      return std::function<R(typename std::tuple_element<Is, Args>::type ...)>{ 
         [](typename std::tuple_element<Is, Args>::type ...) { 
            roxe_assert(false, "unsupported intrinsic"); return (R)0;
         }
      };
   }   
...
#define GENERATE_TYPE_MAPPING(name) \
   struct __ ## name ## _types { \
      using deduced_full_ts = decltype(roxe::native::get_args_full(::name)); \
      using deduced_ts      = decltype(roxe::native::get_args(::name)); \
...
#define GET_TYPE(name) \
   decltype(create_function<roxe::native::intrinsics::__ ## name ## _types::res_t, \
         roxe::native::intrinsics::__ ## name ## _types::deduced_full_ts>(roxe::native::intrinsics::__ ## name ## _types::is)),

#define REGISTER_INTRINSIC(name) \
   create_function<roxe::native::intrinsics::__ ## name ## _types::res_t, \
         roxe::native::intrinsics::__ ## name ## _types::deduced_full_ts>(roxe::native::intrinsics::__ ## name ## _types::is),
