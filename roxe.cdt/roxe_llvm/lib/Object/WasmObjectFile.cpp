...
Error WasmObjectFile::parseRoxeABISection(ReadContext& Ctx) {
   StringRef sr = readString(Ctx);
   roxe_abi = sr;

   if (Ctx.Ptr != Ctx.End)
      return make_error<GenericBinaryError>("roxe abi section ended prematurely",
                                          object_error::parse_failed);
   return Error::success();
...
Error WasmObjectFile::parseCustomSection(WasmSection &Sec, ReadContext &Ctx) {
  if (Sec.Name == ".imports") {
     if (Error Err = parseAllowedSection(Ctx))
        return Err;
  } else if (Sec.Name == ".roxe_abi") {
     if (Error Err = parseRoxeABISection(Ctx))
        return Err;
  } else if (Sec.Name == ".roxe_actions") {
     if (Error Err = parseActionsSection(Ctx))
        return Err;
  } else if (Sec.Name == ".roxe_notify") {
