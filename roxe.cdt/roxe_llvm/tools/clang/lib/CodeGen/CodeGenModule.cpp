...
llvm::Constant *CodeGenModule::GetOrCreateLLVMFunction(
  ...
  if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
     if (FD->hasAttr<RoxeWasmImportAttr>())
        isWasmImport = true;
     if (FD->hasAttr<RoxeWasmEntryAttr>())
        isWasmEntry = true;
     if (FD->hasAttr<RoxeWasmABIAttr>())
        isWasmABI = true;
     if (FD->hasAttr<RoxeWasmActionAttr>())
        isWasmAction = true;
     if (FD->hasAttr<RoxeWasmNotifyAttr>())
        isWasmNotify = true;
  ...
  if (isWasmImport)
   F->addFnAttr("roxe_wasm_import", "true");

  if (isWasmEntry)
   F->addFnAttr("roxe_wasm_entry", "true");
     
  if (isWasmABI)
     if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
        F->addFnAttr("roxe_wasm_abi", FD->getWasmABI().c_str());
     }
  if (isWasmAction)
     if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
        F->addFnAttr("roxe_wasm_action", FD->getRoxeWasmAction().c_str());
     }
  if (isWasmNotify)
     if (const FunctionDecl *FD = cast_or_null<FunctionDecl>(D)) {
        F->addFnAttr("roxe_wasm_notify", FD->getRoxeWasmNotify().c_str());
     }
