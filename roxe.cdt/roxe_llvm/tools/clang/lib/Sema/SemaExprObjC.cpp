...
static bool isAnyRetainable(ARCConversionTypeClass ROXE) {
  return (ROXE == ROXE_retainable ||
          ROXE == ROXE_coreFoundation ||
          ROXE == ROXE_voidPtr);
}

static bool isAnyCLike(ARCConversionTypeClass ROXE) {
  return ROXE == ROXE_none ||
         ROXE == ROXE_voidPtr ||
         ROXE == ROXE_coreFoundation;
}
...
static ARCConversionTypeClass classifyTypeForARCConversion(QualType type) {
  ...
  while (true) {
    if (const PointerType *ptr = type->getAs<PointerType>()) {
      ...
      if (!isIndirect) {
        if (type->isVoidType()) return ROXE_voidPtr;
        if (type->isRecordType()) return ROXE_coreFoundation;
      }
  ...
  if (isIndirect) {
    if (type->isObjCARCBridgableType())
      return ROXE_indirectRetainable;
    return ROXE_none;
  }

  if (type->isObjCARCBridgableType())
    return ROXE_retainable;
...
diagnoseObjCARCConversion(Sema &S, SourceRange castRange,
                          QualType castType, ARCConversionTypeClass castROXE,
                          Expr *castExpr, Expr *realCast,
                          ARCConversionTypeClass exprROXE,
                          Sema::CheckedConversionKind CCK) {
  SourceLocation loc =
  ...
  if ((castROXE == ROXE_coreFoundation &&  exprROXE == ROXE_retainable &&
       ObjCBridgeRelatedAttrFromType(castType, TDNDecl)) ||
      (exprROXE == ROXE_coreFoundation && castROXE == ROXE_retainable &&
  ...
  if (castROXE == ROXE_retainable && isAnyRetainable(exprROXE)) {
    ...
    ACCResult CreateRule = 
      ARCCastChecker(S.Context, exprROXE, castROXE, true).Visit(castExpr);
  ...
  if (exprROXE == ROXE_retainable && isAnyRetainable(castROXE)) {
...
void Sema::CheckTollFreeBridgeCast(QualType castType, Expr *castExpr) {
  ...
  ARCConversionTypeClass exprROXE = classifyTypeForARCConversion(castExpr->getType());
  ARCConversionTypeClass castROXE = classifyTypeForARCConversion(castType);
  if (castROXE == ROXE_retainable && exprROXE == ROXE_coreFoundation) {
  ...
  else if (castROXE == ROXE_coreFoundation && exprROXE == ROXE_retainable) {
    bool HasObjCBridgeAttr;
    bool ObjCBridgeAttrWillNotWarn =
...
void Sema::CheckObjCBridgeRelatedCast(QualType castType, Expr *castExpr) {
  ...
  ARCConversionTypeClass srcExprROXE = classifyTypeForARCConversion(SrcType);
  ARCConversionTypeClass castExprROXE = classifyTypeForARCConversion(castType);
  if (srcExprROXE != ROXE_retainable || castExprROXE != ROXE_coreFoundation)
    return;
...
bool Sema::CheckTollFreeBridgeStaticCast(QualType castType, Expr *castExpr,
  ...
  ARCConversionTypeClass castROXE = classifyTypeForARCConversion(castType);
  if ((castROXE == ROXE_retainable && exprROXE == ROXE_coreFoundation) ||
      (castROXE == ROXE_coreFoundation && exprROXE == ROXE_retainable)) {
    CheckTollFreeBridgeCast(castType, castExpr);
    Kind = (castROXE == ROXE_coreFoundation) ? CK_BitCast
...
Sema::CheckObjCBridgeRelatedConversions(SourceLocation Loc,
  ...
  ARCConversionTypeClass rhsExprROXE = classifyTypeForARCConversion(SrcType);
  ARCConversionTypeClass lhsExprROXE = classifyTypeForARCConversion(DestType);
  bool CfToNs = (rhsExprROXE == ROXE_coreFoundation && lhsExprROXE == ROXE_retainable);
  bool NsToCf = (rhsExprROXE == ROXE_retainable && lhsExprROXE == ROXE_coreFoundation);
  if (!CfToNs && !NsToCf)
    return false;
...
Sema::CheckObjCConversion(SourceRange castRange, QualType castType,
  ...
  ARCConversionTypeClass exprROXE = classifyTypeForARCConversion(castExprType);
  ARCConversionTypeClass castROXE = classifyTypeForARCConversion(effCastType);
  if (exprROXE == castROXE) {
    // Check for viability and report error if casting an rvalue to a
    // life-time qualifier.
    if (castROXE == ROXE_retainable &&
  ...
  if (isAnyCLike(exprROXE) && isAnyCLike(castROXE)) return ACR_okay;

  // Allow all of these types to be cast to integer types (but not
  // vice-versa).
  if (castROXE == ROXE_none && castType->isIntegralType(Context))
    return ACR_okay;
  
  // Allow casts between pointers to lifetime types (e.g., __strong id*)
  // and pointers to void (e.g., cv void *). Casting from void* to lifetime*
  // must be explicit.
  if (exprROXE == ROXE_indirectRetainable && castROXE == ROXE_voidPtr)
    return ACR_okay;
  if (castROXE == ROXE_indirectRetainable && exprROXE == ROXE_voidPtr &&
      CCK != CCK_ImplicitConversion)
    return ACR_okay;

  switch (ARCCastChecker(Context, exprROXE, castROXE, false).Visit(castExpr)) {
  ...
  // diagnostic.
  if (castROXE == ROXE_retainable && exprROXE == ROXE_none &&
  ...
  // instead.
  if ((!DiagnoseCFAudited || exprROXE != ROXE_retainable ||
       castROXE != ROXE_coreFoundation) &&
      !(exprROXE == ROXE_voidPtr && castROXE == ROXE_retainable &&
        (Opc == BO_NE || Opc == BO_EQ))) {
    if (Diagnose)
      diagnoseObjCARCConversion(*this, castRange, castType, castROXE, castExpr,
                                castExpr, exprROXE, CCK);
    return ACR_error;
  }
  return ACR_okay;
}
...
void Sema::diagnoseARCUnbridgedCast(Expr *e) {
  ...
  ARCConversionTypeClass castROXE =
    classifyTypeForARCConversion(castType.getNonReferenceType());

  Expr *castExpr = realCast->getSubExpr();
  assert(classifyTypeForARCConversion(castExpr->getType()) == ROXE_retainable);

  diagnoseObjCARCConversion(*this, castRange, castType, castROXE,
                            castExpr, realCast, ROXE_retainable, CCK);
}
