...
BOOST_FIXTURE_TEST_CASE( get_scope_test, TESTER ) try {
   ...
   auto act = mutable_variant_object()
         ("issuer",       "roxe")
         ("maximum_supply", roxe::chain::asset::from_string("1000000000.0000 ACI"));
   push_action(N(roxe.token), N(create), N(roxe.token), act );

   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, roxe::chain::asset::from_string("999.0000 ACI") );
   }
   produce_blocks(1);
...
BOOST_FIXTURE_TEST_CASE( get_table_test, TESTER ) try {
   ...
   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, roxe::chain::asset::from_string("10000.0000 ACI") );
   }
   produce_blocks(1);
...
BOOST_FIXTURE_TEST_CASE( get_table_by_seckey_test, TESTER ) try {
   ...
   bidname(N(inita), N(com), roxe::chain::asset::from_string("10.0000 ACI"));
   bidname(N(initb), N(org), roxe::chain::asset::from_string("11.0000 ACI"));
   bidname(N(initc), N(io), roxe::chain::asset::from_string("12.0000 ACI"));
   bidname(N(initd), N(html), roxe::chain::asset::from_string("14.0000 ACI"));
   produce_blocks(1);
