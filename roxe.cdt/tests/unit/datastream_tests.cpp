...
using roxe::binary_extension;
using roxe::datastream;
using roxe::fixed_bytes;
using roxe::ignore;
using roxe::ignore_wrapper;
using roxe::pack;
using roxe::pack_size;
...
struct be_test {
   ...
   be_test(int i) : val{i} {}
   int val;
   ROXELIB_SERIALIZE( be_test, (val) )
};

// Definitions in `roxe.cdt/libraries/roxe/datastream.hpp`
ROXE_TEST_BEGIN(datastream_test)
...
// Definitions in `roxe.cdt/libraries/roxe/datastream.hpp`
ROXE_TEST_BEGIN(datastream_specialization_test)
   static constexpr uint16_t buffer_size{256};
   char datastream_buffer[buffer_size]{}; // Buffer for the datastream to point to
   char buffer[buffer_size]; // Buffer to compare `datastream_buffer` with
...
// Definitions in `roxe.cdt/libraries/roxe/datastream.hpp`
ROXE_TEST_BEGIN(datastream_stream_test)
   static constexpr uint16_t buffer_size{256};
   char datastream_buffer[buffer_size]; // Buffer for the datastream to point to
...
// Definitions in `roxe.cdt/libraries/roxe/datastream.hpp`
ROXE_TEST_BEGIN(misc_datastream_test)
   // ---------------------------
...
int main(int argc, char* argv[]) {
   ...
   ROXE_TEST(datastream_test);
   ROXE_TEST(datastream_specialization_test);
   ROXE_TEST(datastream_stream_test);
   ROXE_TEST(misc_datastream_test);
   return has_failed();
}
