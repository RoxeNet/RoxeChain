...
namespace {
  // RoxeApply - Mutate the apply function as needed
  struct RoxeApplyPass : public FunctionPass {
    static char ID; 
    RoxeApplyPass() : FunctionPass(ID) {}
    bool runOnFunction(Function &F) override {
       if (F.hasFnAttribute("roxe_wasm_entry") || F.getName().equals("apply")) {
...
char RoxeApplyPass::ID = 0;
static RegisterPass<RoxeApplyPass> X("apply_fixup", "Roxe Apply Fixups");

static void registerRoxeApplyPass(const PassManagerBuilder&, legacy::PassManagerBase& PM) { PM.add(new RoxeApplyPass()); }
static RegisterStandardPasses RegisterMyPass(PassManagerBuilder::EP_EarlyAsPossible, registerRoxeApplyPass);
