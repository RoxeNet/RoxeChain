...
struct generation_utils {
      ...
      auto check = [&](const clang::Type* pt) {
        if (auto tst = llvm::dyn_cast<clang::TemplateSpecializationType>(pt))
         if (auto rt = llvm::dyn_cast<clang::RecordType>(tst->desugar()))
            return rt->getDecl()->isRoxeIgnore();
      ...
      auto get = [&](const clang::Type* pt) {
         if (auto tst = llvm::dyn_cast<clang::TemplateSpecializationType>(pt))
            if (auto decl = llvm::dyn_cast<clang::RecordType>(tst->desugar()))
               return decl->getDecl()->isRoxeIgnore() ? tst->getArg(0).getAsType() : type;
         return type;
      };
   ...
   static inline bool has_roxe_ricardian( const clang::CXXMethodDecl* decl ) {
      return decl->hasRoxeRicardian();
   }
   static inline bool has_roxe_ricardian( const clang::CXXRecordDecl* decl ) {
      return decl->hasRoxeRicardian();
   }

   static inline std::string get_roxe_ricardian( const clang::CXXMethodDecl* decl ) {
      return decl->getRoxeRicardianAttr()->getName();
   }
   static inline std::string get_roxe_ricardian( const clang::CXXRecordDecl* decl ) {
      return decl->getRoxeRicardianAttr()->getName();
   }

   static inline std::string get_action_name( const clang::CXXMethodDecl* decl ) {
      std::string action_name = "";
      auto tmp = decl->getRoxeActionAttr()->getName();
      if (!tmp.empty())
         return tmp;
   ...
   static inline std::string get_notify_pair( const clang::CXXMethodDecl* decl ) {
      ...
      auto tmp = decl->getRoxeNotifyAttr()->getName();
      return tmp;
   }
   ...
   static inline bool is_roxe_contract( const clang::CXXMethodDecl* decl, const std::string& cn ) {
      std::string name = "";
      if (decl->isRoxeContract())
         name = decl->getRoxeContractAttr()->getName();
      else if (decl->getParent()->isRoxeContract())
         name = decl->getParent()->getRoxeContractAttr()->getName();
      if (name.empty()) {
   ...
   static inline bool is_roxe_contract( const clang::CXXRecordDecl* decl, const std::string& cn ) {
      std::string name = "";
      auto pd = llvm::dyn_cast<clang::CXXRecordDecl>(decl->getParent());
      if (decl->isRoxeContract()) {
         auto nm = decl->getRoxeContractAttr()->getName().str();
         name = nm.empty() ? decl->getName().str() : nm;
      }
      else if (pd && pd->isRoxeContract()) {
         auto nm = pd->getRoxeContractAttr()->getName().str();
