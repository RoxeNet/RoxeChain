...
#include <roxe/gen.hpp>

#include <roxe/utils.hpp>
#include <roxe/whereami/whereami.hpp>
#include <roxe/abi.hpp>
...
using namespace roxe::cdt;

namespace roxe { namespace cdt {
   ...
   struct codegen_exception : public std::exception {
      virtual const char* what() const throw() {
         return "roxe.codegen fatal error";
      }
   };
   ...
   // remove after v1.7.0
   bool has_roxelib = false;

   class roxe_ppcallbacks : public PPCallbacks {
      public:
         roxe_ppcallbacks(SourceManager& sm, std::string file) : sources(sm), fn(file) {}
      protected:
         virtual void InclusionDirective(
            ...
            if ( file_name.find("roxelib") != StringRef::npos )
               has_roxelib = true;
         }
   ...
   class roxe_codegen_visitor : public RecursiveASTVisitor<roxe_codegen_visitor>, public generation_utils {
      private:
      ...
      public:
         ...
         explicit roxe_codegen_visitor(CompilerInstance *CI)
         ...
         bool is_datastream(const QualType& qt) {
            ...
            auto ds_re    = std::regex("(((class roxe::)?datastream<[a-zA-Z]+[a-zA-Z0-9]*.*>)|(DataStream)) &");
         ...
         */
         ...
         void create_dispatch(const std::string& attr, const std::string& func_name, F&& get_str, CXXMethodDecl* decl) {
            ...
            if (cg.is_roxe_contract(decl, cg.contract_name)) {
               if (has_roxelib) {
                  ss << "\n\n#include <roxelib/datastream.hpp>\n";
                  ss << "#include <roxelib/name.hpp>\n";
               } else {
                  ss << "\n\n#include <roxe/datastream.hpp>\n";
                  ss << "#include <roxe/name.hpp>\n";
               }
               ss << "extern \"C\" {\n";
               ...
               ss << "}\n";
               ss << "roxe::datastream<const char*> ds{(char*)buff, as};\n";
               int i=0;
               ...
               ss << decl->getParent()->getQualifiedNameAsString() << "{roxe::name{r},roxe::name{c},ds}." << decl->getNameAsString() << "(";
         ...
         void create_action_dispatch(CXXMethodDecl* decl) {
            ...
            create_dispatch("roxe_wasm_action", "__roxe_action_", func, decl);
         }

         void create_notify_dispatch(CXXMethodDecl* decl) {
            auto func = [](CXXMethodDecl* d) { return generation_utils::get_notify_pair(d); };
            create_dispatch("roxe_wasm_notify", "__roxe_notify_", func, decl);
         }
         ...
         virtual bool VisitCXXMethodDecl(CXXMethodDecl* decl) {
            ...
            if (decl->isRoxeAction()) {
               name = generation_utils::get_action_name(decl);
               validate_name(name, [&]() {emitError(*ci, decl->getLocation(), "action not a valid roxe name");});
      ...
      class roxe_codegen_consumer : public ASTConsumer {
      private:
         roxe_codegen_visitor *visitor;
         std::string main_file;
         CompilerInstance* ci;

      public:
         explicit roxe_codegen_consumer(CompilerInstance *CI, std::string file)
            : visitor(new roxe_codegen_visitor(CI)), main_file(file), ci(CI) { }
         ...
         virtual void HandleTranslationUnit(ASTContext &Context) {
            ...
            if (main_fe) {
               ...
               try {
                  ...
                  std::stringstream ss;
                  ss << "extern \"C\" {\n";
                  ss << "void roxe_assert_code(uint32_t, uint64_t);";
                  ss << "\t__attribute__((weak, roxe_wasm_entry, roxe_wasm_abi(";
                  std::string abi = cg.abi;
                  ss << "\"" << _quoted(abi) << "\"";
                  ss << ")))\n";
                  ss << "\tvoid __insert_roxe_abi(unsigned long long r, unsigned long long c, unsigned long long a){";
                  ss << "roxe_assert_code(false, 1);";
                  ss << "}\n";
                  ss << "}";
      ...
      class roxe_codegen_frontend_action : public ASTFrontendAction {
      public:
         virtual std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &CI, StringRef file) {
            CI.getPreprocessor().addPPCallbacks(_make_unique<roxe_ppcallbacks>(CI.getSourceManager(), file.str()));
            return _make_unique<roxe_codegen_consumer>(&CI, file);
         }
   };
