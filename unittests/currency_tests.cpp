...
class currency_tester : public TESTER {
   public:
      ...
      asset get_balance(const account_name& account) const {
         return get_currency_balance(N(gls.token), symbol(SY(4,CUR)), account);
      }
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         auto trace = push_action(N(gls.token), N(issue), mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      currency_tester()
         :TESTER(),abi_ser(json::from_string(contracts::actc_token_abi().data()).as<abi_def>(), abi_serializer_max_time)
      {
         create_account( N(gls.token));
         set_code( N(gls.token), contracts::actc_token_wasm() );

         auto result = push_action(N(gls.token), N(create), mutable_variant_object()
                 ("issuer",       actc_token)
         ...
         result = push_action(N(gls.token), N(issue), mutable_variant_object()
                 ("to",       actc_token)
...
const std::string currency_tester::actc_token = name(N(gls.token)).to_string();
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   currency_tester t;
   auto actual = t.get_currency_balance(N(gls.token), expected.get_symbol(), N(gls.token));
...
BOOST_FIXTURE_TEST_CASE( test_transfer, currency_tester ) try {
   ...
   {
      auto trace = push_action(N(gls.token), N(transfer), mutable_variant_object()
         ("from", actc_token)
         ("to",   "alice")
...
BOOST_FIXTURE_TEST_CASE( test_duplicate_transfer, currency_tester ) {
   ...
   BOOST_REQUIRE_THROW(push_action(N(gls.token), N(transfer), mutable_variant_object()
                                    ("from", actc_token)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
...
BOOST_FIXTURE_TEST_CASE(test_symbol, TESTER) try {
   ...
   {
      symbol sys(4, "LSC");
      BOOST_REQUIRE_EQUAL(SY(4,LSC), sys.value());
      BOOST_REQUIRE_EQUAL("4,LSC", sys.to_string());
      BOOST_REQUIRE_EQUAL("LSC", sys.name());
