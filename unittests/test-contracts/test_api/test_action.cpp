...
void test_action::test_dummy_action() {
   ...
   actc_assert( act.authorization.back().actor == "testapi"_n, "incorrect permission actor" );
   actc_assert( act.authorization.back().permission == "active"_n, "incorrect permission name" );
   actc_assert( actc::pack_size(act) == static_cast<size_t>(total), "pack_size does not match get_action size" );
   actc_assert( act.account == "testapi"_n, "expected testapi account" );
...
void test_action::require_notice( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   if( receiver == "testapi"_n.value ) {
      actc::require_recipient( "acc1"_n );
      actc::require_recipient( "acc2"_n );
      actc::require_recipient( "acc1"_n, "acc2"_n );
...
void test_action::require_notice_tests( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   if( receiver == "testapi"_n.value ) {
      actc::print("require_recipient( \"acc5\"_n )");
      actc::require_recipient("acc5"_n);
   } else if( receiver == "acc5"_n.value ) {
      actc::print("require_recipient( \"testapi\"_n )");
      actc::require_recipient("testapi"_n);
   }
}

void test_action::require_auth() {
   prints("require_auth");
   actc::require_auth("acc3"_n);
   actc::require_auth("acc4"_n);
}
...
void test_action::test_current_receiver( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   actc_assert( receiver == cur_rec.value, "the current receiver does not match" );
}
...
void test_action::test_ram_billing_in_notify( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   if( code == receiver ) {
      actc::require_recipient( name{to_notify} );
   } else {
...
void test_action::test_action_ordinal1(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   if (receiver == "testapi"_n.value) {
      print("exec 1");
      actc::require_recipient( "bob"_n ); //-> exec 2 which would then cause execution of 4, 10

      actc::action act1({name(_self), "active"_n}, name(_self),
      ...
      if (is_account("fail1"_n)) {
         actc_assert(false, "fail at point 1");
      }

      actc::action act2({name(_self), "active"_n}, name(_self),
                         name(WASM_TEST_ACTION("test_action", "test_action_ordinal3")),
                         std::tuple<>());
      act2.send(); // -> exec 9

      actc::require_recipient( "charlie"_n ); // -> exec 3 which would then cause execution of 11
      ...
      actc::require_recipient( "david"_n );  // -> exec 4
      ...
      if (is_account("fail3"_n)) {
         actc_assert(false, "fail at point 3");
      }

   } else if (receiver == "david"_n.value) {
      print("exec 4");
   } else {
      actc_assert(false, "assert failed at test_action::test_action_ordinal1");
   }
}
void test_action::test_action_ordinal2(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   if (receiver == "testapi"_n.value) {
      print("exec 5");
      actc::require_recipient( "david"_n ); // -> exec 6
      actc::require_recipient( "erin"_n ); // -> exec 7
      ...
      actc_assert(false, "assert failed at test_action::test_action_ordinal2");
   }
}
...
void test_action::test_action_ordinal3(uint64_t receiver, uint64_t code, uint64_t action) {
   ...
   if (is_account("failnine"_n)) {
      actc_assert(false, "fail at point 9");
   }
}
