...
using namespace actc::internal_use_do_not_use;
...
void test_api_db::primary_i64_general()
{
   ...
   // find
   {
      ...
      actc_assert( itr_next == itr_next_expected && prim == "allyson"_n.value, "primary_i64_general - db_find_i64"  );
      itr_next = db_next_i64( itr_next, &prim );
      itr_next_expected = db_find_i64( receiver, receiver, table1, "bob"_n.value );
      actc_assert( itr_next == itr_next_expected && prim == "bob"_n.value, "primary_i64_general - db_next_i64" );
   }
   ...
   // previous
   {
      ...
      actc_assert( itr_prev == itr_prev_expected && prim == "bob"_n.value, "primary_i64_general - db_previous_i64" );

      itr_prev = db_previous_i64( itr_prev, &prim );
      itr_prev_expected = db_find_i64( receiver, receiver, table1, "allyson"_n.value );
      actc_assert( itr_prev == itr_prev_expected && prim == "allyson"_n.value, "primary_i64_general - db_previous_i64" );

      itr_prev = db_previous_i64( itr_prev, &prim );
      itr_prev_expected = db_find_i64( receiver, receiver, table1, "alice"_n.value );
      actc_assert( itr_prev == itr_prev_expected && prim == "alice"_n.value, "primary_i64_general - db_previous_i64" );

      itr_prev = db_previous_i64( itr_prev, &prim );
      actc_assert( itr_prev < 0 && prim == "alice"_n.value, "primary_i64_general - db_previous_i64" );
   }
...
void test_api_db::primary_i64_lowerbound()
{
...

   {
      ...
      actc_assert( lb == db_find_i64(receiver, receiver, table, "alice"_n.value), err.c_str() );
   }
   {
      int lb = db_lowerbound_i64( receiver, receiver, table, "billy"_n.value );
      actc_assert( lb == db_find_i64(receiver, receiver, table, "bob"_n.value), err.c_str() );
   }
   {
      int lb = db_lowerbound_i64( receiver, receiver, table, "frank"_n.value );
      actc_assert( lb == db_find_i64(receiver, receiver, table, "joe"_n.value), err.c_str() );
   }
   {
...
void test_api_db::primary_i64_upperbound()
{
   ...
   const std::string err = "primary_i64_upperbound";
   {
      ...
      actc_assert( ub == db_find_i64(receiver, receiver, table, "allyson"_n.value), err.c_str() );
   }
   {
      int ub = db_upperbound_i64( receiver, receiver, table, "billy"_n.value );
      actc_assert( ub == db_find_i64(receiver, receiver, table, "bob"_n.value), err.c_str() );
   }
   {
      int ub = db_upperbound_i64( receiver, receiver, table, "frank"_n.value );
      actc_assert( ub == db_find_i64(receiver, receiver, table, "joe"_n.value), err.c_str() );
   }
   {
...
void test_api_db::idx64_general()
{
   ...
   // find_primary
   {
      ...
      actc_assert( itr >= 0 && sec == "joe"_n.value, "idx64_general - db_idx64_find_primary" );
   ...
   // iterate forward starting with charlie
   {
      ...
      actc_assert( itr >= 0 && sec == "charlie"_n.value, "idx64_general - db_idx64_find_primary" );
      ...
      actc_assert( itr_next == itr_next_expected && sec_next == "emily"_n.value, "idx64_general - db_idx64_next" );
      ...
      actc_assert( itr_next == itr_next_expected && sec_next == "joe"_n.value, "idx64_general - db_idx64_next" );
   ...
   // iterate backward staring with second bob
   {
      ...
      actc_assert( itr >= 0 && sec == "bob"_n.value, "idx64_general - db_idx64_find_primary" );
      ...
      actc_assert( itr_prev == itr_prev_expected && sec_prev == "bob"_n.value, "idx64_general - db_idx64_previous" );
      ...
      actc_assert( itr_prev == itr_prev_expected && sec_prev == "allyson"_n.value, "idx64_general - db_idx64_previous" );

      itr_prev = db_idx64_previous( itr_prev, &prim_prev );
      actc_assert( itr_prev >= 0 && prim_prev == 265, "idx64_general - db_idx64_previous" );
      itr_prev_expected = db_idx64_find_primary( receiver, receiver, table, &sec_prev, prim_prev );
      actc_assert( itr_prev == itr_prev_expected && sec_prev == "alice"_n.value, "idx64_general - db_idx64_previous" );
...
void test_api_db::idx64_lowerbound()
{
   ...
   const std::string err = "idx64_lowerbound";
   {
      ...
      actc_assert( lb_prim == ssn && lb_sec == "alice"_n.value, err.c_str() );
   ...
   }
   {
      ...
      int lb = db_idx64_lowerbound( receiver, receiver, table, &lb_sec, &lb_prim );
      actc_assert( lb_prim == ssn && lb_sec == "bob"_n.value, err.c_str() );
      actc_assert( lb == db_idx64_find_primary(receiver, receiver, table, &lb_sec, ssn), err.c_str() );
   }
   {
      ...
      int lb = db_idx64_lowerbound( receiver, receiver, table, &lb_sec, &lb_prim );
      actc_assert( lb_prim == ssn && lb_sec == "joe"_n.value, err.c_str() );
      actc_assert( lb == db_idx64_find_primary(receiver, receiver, table, &lb_sec, ssn), err.c_str() );
   }
   {
      ...
      uint64_t lb_prim = 0;
      int lb = db_idx64_lowerbound( receiver, receiver, table, &lb_sec, &lb_prim );
      actc_assert( lb_prim == 0 && lb_sec == "kevin"_n.value, err.c_str() );
      actc_assert( lb < 0, "" );
   }
}
...
void test_api_db::idx64_upperbound()
{
   ...
   const std::string err = "idx64_upperbound";
   {
      ...
      actc_assert( ub_prim == allyson_ssn && ub_sec == "allyson"_n.value, "" );
   ...
   }
   {
      ...
      int ub = db_idx64_upperbound( receiver, receiver, table, &ub_sec, &ub_prim );
      actc_assert( ub_prim == bob_ssn && ub_sec == "bob"_n.value, "" );
   ...
   }
   {
      ...
      uint64_t ub_prim = 0;
      int ub = db_idx64_upperbound( receiver, receiver, table, &ub_sec, &ub_prim );
      actc_assert( ub_prim == 0 && ub_sec == "joe"_n.value, err.c_str() );
      actc_assert( ub < 0, err.c_str() );
   }
   {
      ...
      uint64_t ub_prim = 0;
      int ub = db_idx64_upperbound( receiver, receiver, table, &ub_sec, &ub_prim );
      actc_assert( ub_prim == 0 && ub_sec == "kevin"_n.value, err.c_str() );
      actc_assert( ub < 0, err.c_str() );
   }
}
...
void test_api_db::test_invalid_access( name _code, uint64_t val, uint32_t index, bool store )
{
   ...
   if(store) {
      ...
      //actc::print("test_invalid_access: expected ", val, " and retrieved ", value, "\n");
      actc_assert( value == val, "test_invalid_access: value did not match" );
   }
}
...
void test_api_db::misaligned_secondary_key256_tests() {
   uint64_t receiver = get_self().value;
   auto key = actc::checksum256::make_from_word_sequence<uint64_t>( 0ULL, 0ULL, 0ULL, 42ULL );
   char* ptr = (char*)(&key);
   ptr += 1;
