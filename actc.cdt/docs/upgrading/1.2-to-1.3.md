...
- Removed the `S` macro. The symbol constructor should be used as a type safe replacement. Example: `S(4,SYS)` -> `symbol(symbol_code("ACI"), 4)` (or simply `symbol("ACI", 4)` as of v1.3.1).
...
- Old contracts that use `actc::asset()` should be changed to either use the core symbol of the specific chain they are targeting i.e. `actc::asset(0, symbol(symbol_code("ACI"),4))`. To reduce writing `symbol(symbol_code("ACI"),4)` over and over, a `constexpr` function to return the symbol or `constexpr` global variable should be used.
...
- New version checking mechanism is included. See actc.contracts/CMakeLists.txt to see this in use.
