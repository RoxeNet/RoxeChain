...
#include <actc/gen.hpp>

#include <actc/utils.hpp>
#include <actc/whereami/whereami.hpp>
#include <actc/abi.hpp>
...
using namespace actc::cdt;

namespace actc { namespace cdt {
   ...
   struct codegen_exception : public std::exception {
      virtual const char* what() const throw() {
         return "actc.codegen fatal error";
      }
   };
   ...
   // remove after v1.7.0
   bool has_actclib = false;

   class actc_ppcallbacks : public PPCallbacks {
      public:
         actc_ppcallbacks(SourceManager& sm, std::string file) : sources(sm), fn(file) {}
      protected:
         virtual void InclusionDirective(
            ...
            if ( file_name.find("actclib") != StringRef::npos )
               has_actclib = true;
         }
   ...
   class actc_codegen_visitor : public RecursiveASTVisitor<actc_codegen_visitor>, public generation_utils {
      private:
      ...
      public:
         ...
         explicit actc_codegen_visitor(CompilerInstance *CI)
         ...
         bool is_datastream(const QualType& qt) {
            ...
            auto ds_re    = std::regex("(((class actc::)?datastream<[a-zA-Z]+[a-zA-Z0-9]*.*>)|(DataStream)) &");
         ...
         */
         ...
         void create_dispatch(const std::string& attr, const std::string& func_name, F&& get_str, CXXMethodDecl* decl) {
            ...
            if (cg.is_actc_contract(decl, cg.contract_name)) {
               if (has_actclib) {
                  ss << "\n\n#include <actclib/datastream.hpp>\n";
                  ss << "#include <actclib/name.hpp>\n";
               } else {
                  ss << "\n\n#include <actc/datastream.hpp>\n";
                  ss << "#include <actc/name.hpp>\n";
               }
               ss << "extern \"C\" {\n";
               ...
               ss << "}\n";
               ss << "actc::datastream<const char*> ds{(char*)buff, as};\n";
               int i=0;
               ...
               ss << decl->getParent()->getQualifiedNameAsString() << "{actc::name{r},actc::name{c},ds}." << decl->getNameAsString() << "(";
         ...
         void create_action_dispatch(CXXMethodDecl* decl) {
            ...
            create_dispatch("actc_wasm_action", "__actc_action_", func, decl);
         }

         void create_notify_dispatch(CXXMethodDecl* decl) {
            auto func = [](CXXMethodDecl* d) { return generation_utils::get_notify_pair(d); };
            create_dispatch("actc_wasm_notify", "__actc_notify_", func, decl);
         }
      ...
      class actc_codegen_consumer : public ASTConsumer {
      private:
         actc_codegen_visitor *visitor;
         std::string main_file;
         CompilerInstance* ci;

      public:
         explicit actc_codegen_consumer(CompilerInstance *CI, std::string file)
            : visitor(new actc_codegen_visitor(CI)), main_file(file), ci(CI) { }
         ...
         virtual void HandleTranslationUnit(ASTContext &Context) {
            ...
            if (main_fe) {
               ...
               try {
                  ...
                  std::stringstream ss;
                  ss << "extern \"C\" {\n";
                  ss << "void actc_assert_code(uint32_t, uint64_t);";
                  ss << "\t__attribute__((weak, actc_wasm_entry, actc_wasm_abi(";
                  std::string abi = cg.abi;
                  ss << "\"" << _quoted(abi) << "\"";
                  ss << ")))\n";
                  ss << "\tvoid __insert_actc_abi(unsigned long long r, unsigned long long c, unsigned long long a){";
                  ss << "actc_assert_code(false, 1);";
                  ss << "}\n";
                  ss << "}";
      ...
      class actc_codegen_frontend_action : public ASTFrontendAction {
      public:
         virtual std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &CI, StringRef file) {
            CI.getPreprocessor().addPPCallbacks(_make_unique<actc_ppcallbacks>(CI.getSourceManager(), file.str()));
            return _make_unique<actc_codegen_consumer>(&CI, file);
         }
   };
