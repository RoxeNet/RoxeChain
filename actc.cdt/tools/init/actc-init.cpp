...
struct project {
   ...
   const std::string hpp = "#include <actc/actc.hpp>\n"
                           "using namespace actc;\n\n"
   ...
   const std::string cmake = "project(@)\n\n"
                             "set(ACTC_WASM_OLD_BEHAVIOR \"Off\")\n"
   ...
   const std::string cmake_extern = "include(ExternalProject)\n"
                                    ...
                                    "if(ACTC_CDT_ROOT STREQUAL \"\" OR NOT ACTC_CDT_ROOT)\n"
                                    "   find_package(actc.cdt)\n"
                                    "endif()\n\n"
                                    "ExternalProject_Add(\n"
                                    "   @_project\n"
                                    ...
                                    "   CMAKE_ARGS -DCMAKE_TOOLCHAIN_FILE=${ACTC_CDT_ROOT}/lib/cmake/actc.cdt/EosioWasmToolchain.cmake\n"
                                    "   UPDATE_COMMAND \"\"\n"
                                    "   PATCH_COMMAND \"\"\n"
                                    "   TEST_COMMAND \"\"\n"
                                    "   INSTALL_COMMAND \"\"\n"
   ...
   const std::string readme_cmake = "--- @ Project ---\n\n"
                                    ...
                                    "   - You can then do a 'set contract' action with 'clactc' and point in to the './build/@' directory\n\n"
                                    " - Additions to CMake should be done to the CMakeLists.txt in the './src' directory and not in the top level CMakeLists.txt";

   const std::string readme_bare = " --- @ Project ---\n\n"
                                   " - How to Build -\n"
                                   "   - run the command 'actc-cpp -abigen -o @.wasm @.cpp'\n";
...
int main(int argc, const char **argv) {
   ...
   cl::SetVersionPrinter([](llvm::raw_ostream& os) {
        os << "actc-init version " << "@VERSION_FULL@" << "\n";
  });
   cl::OptionCategory cat("actc-init", "generates an actc smart contract project");
   ...
   cl::ParseCommandLineOptions(argc, argv, std::string("actc-proj"));
   try {
