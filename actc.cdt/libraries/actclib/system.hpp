...
#warning "<actclib/system.hpp> is deprecated use <actc/system.hpp>"
namespace actc {
   ...
   inline void check(bool pred, const char* msg, size_t n) {
      if (!pred) {
         actc_assert_message(false, msg, n);
      }
   }
   ...
   inline void check(bool pred, const std::string& msg, size_t n) {
      if (!pred) {
         actc_assert_message(false, msg.c_str(), n);
      }
   }
