...
#warning "<actclib/name.hpp> is deprecated use <actc/name.hpp>"
namespace actc {
   ...
   public:
      ...
      {
         if( str.size() > 13 ) {
            actc::check( false, "string is too long to be a valid name" );
         }
         if( str.empty() ) {
            return;
         }
         ...
         if( str.size() == 13 ) {
            ...
            if( v > 0x0Full ) {
               actc::check(false, "thirteenth character in name cannot be a letter that comes after j");
            }
            value |= v;
         }
      }
      ...
      static constexpr uint8_t char_to_value( char c ) {
         if( c == '.')
            ...
            actc::check( false, "character is not in allowed character set for names" );
...
inline constexpr actc::name operator""_n() {
   constexpr auto x = actc::name{std::string_view{actc::detail::to_const_char_arr<Str...>::value, sizeof...(Str)}};
   return x;
}
