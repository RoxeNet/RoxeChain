...
#warning "<actclib/asset.hpp> is deprecated use <actc/asset.hpp>"
...
namespace actc {
   ...
   struct asset {
      ...
      :amount(a),symbol{s}
      {
         actc::check( is_amount_within_range(), "magnitude of asset amount must be less than 2^62" );
         actc::check( symbol.is_valid(),        "invalid symbol name" );
      }
      ...
      asset& operator-=( const asset& a ) {
         actc::check( a.symbol == symbol, "attempt to subtract asset with different symbol" );
         amount -= a.amount;
         actc::check( -max_amount <= amount, "subtraction underflow" );
         actc::check( amount <= max_amount,  "subtraction overflow" );
         return *this;
      }
       ...
       */
      asset& operator+=( const asset& a ) {
         actc::check( a.symbol == symbol, "attempt to add asset with different symbol" );
         amount += a.amount;
         actc::check( -max_amount <= amount, "addition underflow" );
         actc::check( amount <= max_amount,  "addition overflow" );
         return *this;
      }
      ...
      asset& operator*=( int64_t a ) {
         ...
         actc::check( tmp <= max_amount, "multiplication overflow" );
         actc::check( tmp >= -max_amount, "multiplication underflow" );
         amount = (int64_t)tmp;
         return *this;
      }
       ...
       */
      asset& operator/=( int64_t a ) {
         actc::check( a != 0, "divide by zero" );
         actc::check( !(amount == std::numeric_limits<int64_t>::min() && a == -1), "signed division overflow" );
         amount /= a;
         return *this;
      }
      ...
      friend int64_t operator/( const asset& a, const asset& b ) {
         actc::check( b.amount != 0, "divide by zero" );
         actc::check( a.symbol == b.symbol, "comparison of assets with different symbols is not allowed" );
   ...
   */
   struct extended_asset {
      ...
      friend extended_asset operator - ( const extended_asset& a, const extended_asset& b ) {
         actc::check( a.contract == b.contract, "type mismatch" );
