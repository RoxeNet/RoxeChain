...
#include "core/actc/powers.hpp"
#include "contracts/actc/system.hpp"
#include "contracts/actc/privileged.hpp"
...
namespace actc {
   ...
   void get_blockchain_parameters(actc::blockchain_parameters& params) {
      ...
      actc::check( size <= sizeof(buf), "buffer is too small" );
   ...
   std::optional<uint64_t> set_proposed_producers( const std::vector<producer_key>& prods ) {
      auto packed_prods = actc::pack( prods );
