...
#warning "<actclib/binary_extension.hpp> is deprecated use <actc/binary_extension.hpp>"
 ...
 namespace actc {
   ...
   class binary_extension {
      public:
         ...
         constexpr T& value()& {
            if (!_has_value) {
               actc_assert(false, "cannot get value of empty binary_extension");
            }
            return _get();
         }
          /** get the contained value */
         ...
         **/
         ...
         friend inline DataStream& operator<<(DataStream& ds, const actc::binary_extension<T>& be) {
            ds << be.value_or();
            return ds;
         }
         ...
         /**
           ...
           */
         template<typename DataStream>
         friend inline DataStream& operator>>(DataStream& ds, actc::binary_extension<T>& be) {
            if( ds.remaining() ) {
               T val;
               ds >> val;
               be.emplace(val);
