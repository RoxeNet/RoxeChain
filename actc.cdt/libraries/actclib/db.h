...
#warning "<actclib/db.h> is deprecated use <actc/db.h>. If you are using C++ the .h header files will be removed from inclusion entirely in v1.7.0"

/**
 *  @addtogroup database_c_api Database C API
 *  @ingroup c_api
 *  @brief Defines %C APIs for interfacing with the database.
 *  @details Database C API provides low level interface to ACTC database.
 *
...
/**
  ...
  *  actc_assert(itr >= 0, "Alice cannot be removed since she was already not found in the table");
  *  db_remove_i64(itr);
  *  @endcode
  */
__attribute__((actc_wasm_import))
...
/**
  ...
  *  actc_assert(len <= 50, "buffer to small to store retrieved record");
...
/**
  ...
  *  actc_assert(end_itr < -1, "Charlie was not the last entry in the table");
  *  @endcode
  */
__attribute__((actc_wasm_import))
