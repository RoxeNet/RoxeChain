#include <actc/action.h>
#include <actc/chain.h>
#include <actc/crypto.h>
#include <actc/db.h>
#include <actc/permission.h>
#include <actc/print.h>
#include <actc/privileged.h>
#include <actc/system.h>
#include <actc/transaction.h>
#include <actc/types.h>
#include "native/actc/intrinsics.hpp"
#include "native/actc/crt.hpp"
...
extern "C" {
   ...
   int32_t _actc_f32_trunc_i32s( float af ) {
      ...
      if (_actc_f32_ge(af, 2147483648.0f) || _actc_f32_lt(af, -2147483648.0f))
         actc_assert(false,  "Error, f32.convert_s/i32 overflow" );

      if (f32_is_nan(a))
         actc_assert(false,  "Error, f32.convert_s/i32 unrepresentable");
   ...
   int32_t _actc_f64_trunc_i32s( double af ) {
      ...
      if (_actc_f64_ge(af, 2147483648.0) || _actc_f64_lt(af, -2147483648.0))
         actc_assert(false,  "Error, f64.convert_s/i32 overflow");
      if (f64_is_nan(a))
         actc_assert(false,  "Error, f64.convert_s/i32 unrepresentable");
   ...
   uint32_t _actc_f32_trunc_i32u( float af ) {
      ...
      if (_actc_f32_ge(af, 4294967296.0f) || _actc_f32_le(af, -1.0f))
         actc_assert(false,  "Error, f32.convert_u/i32 overflow");
      if (f32_is_nan(a))
         actc_assert(false,  "Error, f32.convert_u/i32 unrepresentable");
   ...
   uint32_t _actc_f64_trunc_i32u( double af ) {
      ...
      if (_actc_f64_ge(af, 4294967296.0) || _actc_f64_le(af, -1.0))
         actc_assert(false,  "Error, f64.convert_u/i32 overflow");
      if (f64_is_nan(a))
         actc_assert(false,  "Error, f64.convert_u/i32 unrepresentable");
   ...
   int64_t _actc_f32_trunc_i64s( float af ) {
      ...
      if (_actc_f32_ge(af, 9223372036854775808.0f) || _actc_f32_lt(af, -9223372036854775808.0f))
         actc_assert(false,  "Error, f32.convert_s/i64 overflow");
      if (f32_is_nan(a))
         actc_assert(false,  "Error, f32.convert_s/i64 unrepresentable");
   ...
   int64_t _actc_f64_trunc_i64s( double af ) {
      ...
      if (_actc_f64_ge(af, 9223372036854775808.0) || _actc_f64_lt(af, -9223372036854775808.0))
         actc_assert(false,  "Error, f64.convert_s/i64 overflow");
      if (f64_is_nan(a))
         actc_assert(false,  "Error, f64.convert_s/i64 unrepresentable");
   ...
   uint64_t _actc_f32_trunc_i64u( float af ) {
      ...
      if (_actc_f32_ge(af, 18446744073709551616.0f) || _actc_f32_le(af, -1.0f))
         actc_assert(false,  "Error, f32.convert_u/i64 overflow");
      if (f32_is_nan(a))
         actc_assert(false,  "Error, f32.convert_u/i64 unrepresentable");
   ...
   uint64_t _actc_f64_trunc_i64u( double af ) {
      ...
      if (_actc_f64_ge(af, 18446744073709551616.0) || _actc_f64_le(af, -1.0))
         actc_assert(false,  "Error, f64.convert_u/i64 overflow");
      if (f64_is_nan(a))
         actc_assert(false,  "Error, f64.convert_u/i64 unrepresentable");
   ...
   void actc_assert(uint32_t test, const char* msg) {
      if (test == 0) {
         _prints(msg, actc::cdt::output_stream_kind::std_err);
         _prints_l("\n", 1, actc::cdt::output_stream_kind::none);
         longjmp(*___env_ptr, 1);
      }
   }

   void actc_assert_message(uint32_t test, const char* msg, uint32_t len) {
      if (test == 0) {
         _prints_l(msg, len, actc::cdt::output_stream_kind::std_err);
         _prints_l("\n", 1, actc::cdt::output_stream_kind::none);
         longjmp(*___env_ptr, 1);
      }
   }
   ...
   void actc_assert_code(uint32_t test, uint64_t code) {
      if (test == 0) {
         ...
         _prints(buff, actc::cdt::output_stream_kind::std_err);
         _prints_l("\n", 1, actc::cdt::output_stream_kind::none);
         longjmp(*___env_ptr, 1);
      }
   }
