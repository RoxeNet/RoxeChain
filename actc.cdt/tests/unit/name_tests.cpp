...
// Definitions in `actc.cdt/libraries/actc/name.hpp`
ACTC_TEST_BEGIN(name_type_test)
   //// constexpr name()
   ...
   CHECK_EQUAL( name{"actc"}.length(), 3 )
   CHECK_EQUAL( name{"actci"}.length(), 4 )
   CHECK_EQUAL( name{"actc"}.length(), 5 )
   CHECK_EQUAL( name{"actca"}.length(), 6 )
   CHECK_EQUAL( name{"actcac"}.length(), 7 )
   CHECK_EQUAL( name{"actcacc"}.length(), 8 )
   CHECK_EQUAL( name{"actcacco"}.length(), 9 )
   CHECK_EQUAL( name{"actcaccou"}.length(), 10 )
   CHECK_EQUAL( name{"actcaccoun"}.length(), 11 )
   CHECK_EQUAL( name{"actcaccount"}.length(), 12 )
   CHECK_EQUAL( name{"actcaccountj"}.length(), 13 )

   CHECK_ASSERT( "string is too long to be a valid name", ([]() {name{"12345abcdefghj"}.length();}) )

   // ----------------------------
   // constexpr name suffix()const
   CHECK_EQUAL( name{".actcaccounj"}.suffix(), name{"actcaccounj"} )
   CHECK_EQUAL( name{"e.osioaccounj"}.suffix(), name{"osioaccounj"} )
   CHECK_EQUAL( name{"eo.sioaccounj"}.suffix(), name{"sioaccounj"} )
   CHECK_EQUAL( name{"actc.ioaccounj"}.suffix(), name{"ioaccounj"} )
   CHECK_EQUAL( name{"actci.oaccounj"}.suffix(), name{"oaccounj"} )
   CHECK_EQUAL( name{"actc.accounj"}.suffix(), name{"accounj"} )
   CHECK_EQUAL( name{"actca.ccounj"}.suffix(), name{"ccounj"} )
   CHECK_EQUAL( name{"actcac.counj"}.suffix(), name{"counj"} )
   CHECK_EQUAL( name{"actcacc.ounj"}.suffix(), name{"ounj"} )
   CHECK_EQUAL( name{"actcacco.unj"}.suffix(), name{"unj"} )
   CHECK_EQUAL( name{"actcaccou.nj"}.suffix(), name{"nj"} )
   CHECK_EQUAL( name{"actcaccoun.j"}.suffix(), name{"j"} )

   CHECK_EQUAL( name{"e.o.s.i.o.a.c"}.suffix(), name{"c"} )
   CHECK_EQUAL( name{"actc.ioa.cco"}.suffix(), name{"cco"} )
