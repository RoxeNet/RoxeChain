...
Error WasmObjectFile::parseActcABISection(ReadContext& Ctx) {
   StringRef sr = readString(Ctx);
   actc_abi = sr;

   if (Ctx.Ptr != Ctx.End)
      return make_error<GenericBinaryError>("actc abi section ended prematurely",
                                          object_error::parse_failed);
   return Error::success();
...
Error WasmObjectFile::parseCustomSection(WasmSection &Sec, ReadContext &Ctx) {
  if (Sec.Name == ".imports") {
     if (Error Err = parseAllowedSection(Ctx))
        return Err;
  } else if (Sec.Name == ".actc_abi") {
     if (Error Err = parseActcABISection(Ctx))
        return Err;
  } else if (Sec.Name == ".actc_actions") {
     if (Error Err = parseActionsSection(Ctx))
        return Err;
  } else if (Sec.Name == ".actc_notify") {
