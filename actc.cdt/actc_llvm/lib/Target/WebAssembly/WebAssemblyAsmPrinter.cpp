...
void WebAssemblyAsmPrinter::EmitEndOfAsmFile(Module &M) {
  ...
  bool has_abi    = false;
  bool has_actc_action = false;
  bool has_actc_notify = false;
  for (const auto &F : M) {
     if (F.hasFnAttribute("actc_wasm_import"))
        has_import = true;
     if (F.hasFnAttribute("actc_wasm_abi"))
        has_abi = true;
     if (F.hasFnAttribute("actc_wasm_action"))
        has_actc_action = true;
     if (F.hasFnAttribute("actc_wasm_notify"))
        has_actc_notify = true;
  }
  ...
  if (has_abi) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".actc_abi";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("actc_wasm_abi")) {
           StringRef abi = F.getFnAttribute("actc_wasm_abi").getValueAsString();
  ...
  if (has_actc_action) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".actc_actions";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("actc_wasm_action")) {
           StringRef action_name = F.getFnAttribute("actc_wasm_action").getValueAsString();
  ...
  if (has_actc_notify) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".actc_notify";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("actc_wasm_notify")) {
           StringRef name = F.getFnAttribute("actc_wasm_notify").getValueAsString();
