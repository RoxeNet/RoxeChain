 ...
 * ACTC Contracts
 *
 * @details The design of the ACTC blockchain calls for a number of smart contracts that are run at a
...
namespace actc {
   ...
   class [[actc::contract("actc.bios")]] bios : public contract {
      public:
          ...
          */
         [[actc::action]]
         void setprods( std::vector<actc::producer_key> schedule );
          ...
          */
         [[actc::action]]
         void reqactivated( const actc::checksum256& feature_digest );

         struct [[actc::table]] abi_hash {
            name              owner;
            checksum256       hash;
