...
public:
   ...
   void basic_setup() {
      ...
      create_accounts({ N(gls.token), N(gls.ram), N(gls.ramfee), N(gls.stake),
               N(gls.bpay), N(gls.vpay), N(gls.saving), N(gls.names), N(gls.rex) });


      produce_blocks( 100 );
      set_code( N(gls.token), contracts::token_wasm());
   ...
   void create_core_token( symbol core_symbol = symbol{CORE_SYM} ) {
      ...
      create_currency( N(gls.token), config::system_account_name, asset(100000000000000, core_symbol) );
      issue( asset(10000000000000, core_symbol) );
      BOOST_REQUIRE_EQUAL( asset(10000000000000, core_symbol), get_balance( "actc", core_symbol ) );
   }
   ...
   void remaining_setup() {
      ...
      BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"), get_balance("actc")  + get_balance("gls.ramfee") + get_balance("gls.stake") + get_balance("gls.ram") );
   }
   ...
   actc_system_tester( setup_level l = setup_level::full ) {
   ...
   fc::variant get_last_loan(bool cpu) {
      ...
      namespace chain = actc::chain;
      auto table = cpu ? N(cpuloan) : N(netloan);
      const auto* t_id = db.find<actc::chain::table_id_object, chain::by_code_scope_table>( boost::make_tuple( config::system_account_name, config::system_account_name, table ) );
      if ( !t_id ) {
         return fc::variant();
      }
   ...
   fc::variant get_rex_pool() const {
      ...
      const auto& db = control->db();
      namespace chain = actc::chain;
      const auto* t_id = db.find<actc::chain::table_id_object, chain::by_code_scope_table>( boost::make_tuple( config::system_account_name, config::system_account_name, N(rexpool) ) );
      if ( !t_id ) {
         return fc::variant();
      }
   ...
   fc::variant get_rex_return_pool() const {
      ...
      const auto& db = control->db();
      namespace chain = actc::chain;
      const auto* t_id = db.find<actc::chain::table_id_object, chain::by_code_scope_table>( boost::make_tuple( config::system_account_name, config::system_account_name, N(rexretpool) ) );
      if ( !t_id ) {
         return fc::variant();
      }
   ...
   fc::variant get_rex_return_buckets() const {
      ...
      const auto& db = control->db();
      namespace chain = actc::chain;
      const auto* t_id = db.find<actc::chain::table_id_object, chain::by_code_scope_table>( boost::make_tuple( config::system_account_name, config::system_account_name, N(retbuckets) ) );
      if ( !t_id ) {
         return fc::variant();
      }
...
//   }
   ...
   asset get_balance( const account_name& act, symbol balance_symbol = symbol{CORE_SYM} ) {
      vector<char> data = get_row_by_account( N(gls.token), act, N(accounts), balance_symbol.to_symbol_code().value );
   ...
   void issue_and_transfer( const name& to, const asset& amount, const name& manager = config::system_account_name ) {
      signed_transaction trx;
      trx.actions.emplace_back( get_action( N(gls.token), N(issue),
      ...
      if ( to != manager ) {
         trx.actions.emplace_back( get_action( N(gls.token), N(transfer),
                                               vector<permission_level>{{manager, config::active_name}},
                                               mutable_variant_object()
                                               ("from",     manager)
                                               ("to",       to )
   ...
   fc::variant get_global_state2() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "actc_global_state2", data, abi_serializer_max_time );
   }
   ...
   fc::variant get_global_state3() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "actc_global_state3", data, abi_serializer_max_time );
   }
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(gls.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( "actc", "gls.msig", core_sym::from_string("5000.0000") ) );
         produce_block();
   ...
   action_result setinflation( int64_t annual_rate, int64_t inflation_pay_factor, int64_t votepay_factor ) {
      return push_action( N(actc), N(setinflation), mvo()
...
inline uint64_t M( const string& actc_str ) {
   return core_sym::from_string( actc_str ).get_amount();
}
