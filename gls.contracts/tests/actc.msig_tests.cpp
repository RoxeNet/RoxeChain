...
public:
   ...
   transaction_trace_ptr create_account_with_resources( account_name a, account_name creator, asset ramfunds, bool multisig,
      ...
      trx.actions.emplace_back( get_action( N(actc), N(buyram), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("payer", creator)
                                            ("receiver", a)
                                            ("quant", ramfunds) )
      ...
      trx.actions.emplace_back( get_action( N(actc), N(delegatebw), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("from", creator)
                                            ("receiver", a)
   ...
   asset get_balance( const account_name& act ) {
      ...
      const auto* tbl = db.find<table_id_object, by_code_scope_table>(boost::make_tuple(N(gls.token), act, N(accounts)));
      share_type result = 0;
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, actc_msig_tester ) try {
   ...
   set_authority(N(actc), "active", authority(1,
      vector<key_weight>{{get_private_key("actc", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{N(actc.prods), config::active_name}, 1}}), "owner",
      { { N(actc), "active" } }, { get_private_key( N(actc), "active" ) });

   set_producers( {N(alice),N(bob),N(carol)} );
   produce_blocks(50);

   create_accounts( { N(gls.token), N(gls.rex) } );
   set_code( N(gls.token), contracts::token_wasm() );
   set_abi( N(gls.token), contracts::token_abi().data() );

   create_currency( N(gls.token), config::system_account_name, core_sym::from_string("10000000000.0000") );
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"),
                        get_balance("actc") + get_balance("gls.ramfee") + get_balance("gls.stake") + get_balance("gls.ram") );
   ...
   produce_blocks();
   create_account_with_resources( N(alice1111111), N(actc), core_sym::from_string("1.0000"), false );
   create_account_with_resources( N(bob111111111), N(actc), core_sym::from_string("0.4500"), false );
   create_account_with_resources( N(carol1111111), N(actc), core_sym::from_string("1.0000"), false );
   ...
   vector<permission_level> action_perm = {{N(actc), config::active_name}};
   ...
   BOOST_REQUIRE_EXCEPTION( create_account_with_resources( N(alice1111112), N(actc), core_sym::from_string("1.0000"), false ),
                            actc_assert_message_exception, actc_assert_message_is("rejecting all actions")
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_major_approve, actc_msig_tester ) try {
   ...
   create_currency( N(gls.token), config::system_account_name, core_sym::from_string("10000000000.0000") );
   issue(config::system_account_name, core_sym::from_string("1000000000.0000"));
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"), get_balance( "actc" ) );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_invalidate, actc_msig_tester ) try {
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( propose_invalidate_approve, actc_msig_tester ) try {
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( approve_with_hash, actc_msig_tester ) try {
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
   ...
   BOOST_REQUIRE_EXCEPTION( push_action( N(alice), N(approve), mvo()
                            ...
                            actc::chain::crypto_api_exception,
...
BOOST_FIXTURE_TEST_CASE( switch_proposal_and_fail_approve_with_hash, actc_msig_tester ) try {
