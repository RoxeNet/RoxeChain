...
public:
   ...
   actc_wrap_tester() {
      ...
      set_code( N(gls.wrap), contracts::wrap_wasm(), &system_private_key );
      set_abi( N(gls.wrap), contracts::wrap_abi().data(), &system_private_key );
      ...
      const auto& accnt = control->db().get<account_object,by_name>( N(gls.wrap) );
      abi_def abi;
...
BOOST_FIXTURE_TEST_CASE( wrap_exec_direct, actc_wrap_tester ) try {
...

   {
      ...
      wrap_trx.actions.emplace_back( get_action( N(gls.wrap), N(exec),
                                                 {{N(alice), config::active_name}, {N(gls.wrap), config::active_name}},
                                                 mvo()
                                                   ("executer", "alice")
                                                   ("trx", trx)
      ) );
...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig, actc_wrap_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( "gls.wrap", name{traces[0]->action_traces[0].act.account} );
