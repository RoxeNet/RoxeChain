...
BOOST_FIXTURE_TEST_CASE( buysell, actc_system_tester ) try {
   ...
   transfer( "actc", "alice1111111", core_sym::from_string("1000.0000"), "actc" );
   BOOST_REQUIRE_EQUAL( success(), stake( "actc", "alice1111111", core_sym::from_string("200.0000"), core_sym::from_string("100.0000") ) );

   auto total = get_total_stake( "alice1111111" );
   auto init_bytes =  total["ram_bytes"].as_uint64();

   const asset initial_ram_balance = get_balance(N(gls.ram));
   const asset initial_ramfee_balance = get_balance(N(gls.ramfee));
   ...
   BOOST_REQUIRE_EQUAL( initial_ram_balance + core_sym::from_string("199.0000"), get_balance(N(gls.ram)) );
   BOOST_REQUIRE_EQUAL( initial_ramfee_balance + core_sym::from_string("1.0000"), get_balance(N(gls.ramfee)) );
   ...
   transfer( "actc", "alice1111111", core_sym::from_string("100000000.0000"), "actc" );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake, actc_system_tester ) try {
   ...
   const auto init_actc_stake_balance = get_balance( N(gls.stake) );
   ...
   BOOST_REQUIRE_EQUAL( init_actc_stake_balance + core_sym::from_string("300.0000"), get_balance( N(gls.stake) ) );
   ...
   produce_blocks(1);
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000.0000"), get_balance( "alice1111111" ) );
   BOOST_REQUIRE_EQUAL( init_actc_stake_balance, get_balance( N(gls.stake) ) );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake_with_transfer, actc_system_tester ) try {
   ...
   transfer( "actc", "bob111111111", core_sym::from_string("1000.0000"), "actc" );
...
BOOST_FIXTURE_TEST_CASE( fail_without_auth, actc_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "actc", "alice1111111", core_sym::from_string("2000.0000"), core_sym::from_string("1000.0000") ) );
...
//   BOOST_REQUIRE_EQUAL( success(), vote( N(carol1111111), { N(alice1111111) } ) );
   ...
   BOOST_REQUIRE_EQUAL( success(), unstake( "carol1111111", core_sym::from_string("2.0000"), core_sym::from_string("0.0002")/*"2.0000 ACTC", "0.0002 ACTC"*/ ) );
...
//   BOOST_REQUIRE_EQUAL(success(), vote( N(producvotera), { N(defproducera) }));
   ...
   // produce enough blocks so new schedule kicks in and defproducera produces some blocks
   {
      ...
      const int64_t  initial_savings           = get_balance(N(gls.saving)).get_amount();
      ...
      const int64_t  savings           = get_balance(N(gls.saving)).get_amount();
...
BOOST_FIXTURE_TEST_CASE(change_inflation, actc_system_tester) try {
...
//      BOOST_REQUIRE_EQUAL(success(), vote( N(producvotera), { N(defproducera),N(defproducerb),N(defproducerc) }));
      ...
      auto run_for_1year = [this](int64_t annual_rate, int64_t inflation_pay_factor, int64_t votepay_factor) {
         ...
         const int64_t final_savings = get_balance(N(gls.saving)).get_amount();
...
BOOST_AUTO_TEST_CASE(extreme_inflation) try {
   ...
   t.create_currency( N(gls.token), config::system_account_name, asset((1ll << 62) - 1, core_symbol) );
...
//   BOOST_REQUIRE_EQUAL(t.success(), t.vote( N(defproducera), { N(defproducera) }));
   ...
   {
      vector<char> data = t.get_row_by_account( N(gls.token), core_symbol.to_symbol_code().value, N(stat), core_symbol.to_symbol_code().value );
      current_supply = t.token_abi_ser.binary_to_variant("currency_stats", data, actc_system_tester::abi_serializer_max_time)["supply"].template as<asset>();
   }
...
//      BOOST_REQUIRE_EQUAL(success(), vote(N(producvoterd), vector<account_name>(producer_names.begin()+26, producer_names.end())));
...

   {
      ...
      const uint32_t initial_tot_unpaid_blocks = initial_global_state["total_unpaid_blocks"].as<uint32_t>();
      const asset    initial_supply            = get_token_supply();
      const asset    initial_bpay_balance      = get_balance(N(gls.bpay));
      const asset    initial_vpay_balance      = get_balance(N(gls.vpay));
      ...
      const uint32_t tot_unpaid_blocks = global_state["total_unpaid_blocks"].as<uint32_t>();
      const asset    supply            = get_token_supply();
      const asset    bpay_balance      = get_balance(N(gls.bpay));
      const asset    vpay_balance      = get_balance(N(gls.vpay));
...
BOOST_FIXTURE_TEST_CASE(multiple_producer_votepay_share, actc_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...
BOOST_FIXTURE_TEST_CASE(votepay_share_invariant, actc_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...
BOOST_FIXTURE_TEST_CASE(votepay_share_proxy, actc_system_tester, * boost::unit_test::tolerance(1e-5)) try {
...
BOOST_FIXTURE_TEST_CASE(votepay_share_update_order, actc_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...
BOOST_FIXTURE_TEST_CASE(votepay_transition, actc_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...
BOOST_AUTO_TEST_CASE(votepay_transition2, * boost::unit_test::tolerance(1e-10)) try {
   ...
   auto old_core_from_string = [&]( const std::string& s ) {
      return actc::chain::asset::from_string(s + " " + old_contract_core_symbol_name);
   };
   ...
   t.set_abi(  config::system_account_name, contracts::util::system_abi_old().data() );
   {
      ...
      t.abi_ser.set_abi(abi, actc_system_tester::abi_serializer_max_time);
   }
...
//   BOOST_REQUIRE_EQUAL( success(), vote( N(dan), { config::system_account_name } ) );
   ...
   transfer( "actc", "nofail", core_sym::from_string( "1000.0000" ) );
...
//   BOOST_REQUIRE_EQUAL( success(), vote( N(carl), { N(producer) } ) );
   ...
   {
      const asset initial_names_balance = get_balance(N(gls.names));
      BOOST_REQUIRE_EQUAL( success(),
      ...
      BOOST_REQUIRE_EQUAL( initial_names_balance + core_sym::from_string("0.1001"), get_balance(N(gls.names)) );
   }
...
BOOST_FIXTURE_TEST_CASE( namebid_pending_winner, actc_system_tester ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( ram_inflation, actc_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( actcram_ramusage, actc_system_tester ) try {
   BOOST_REQUIRE_EQUAL( core_sym::from_string("0.0000"), get_balance( "alice1111111" ) );
   transfer( "actc", "alice1111111", core_sym::from_string("1000.0000"), "actc" );
   BOOST_REQUIRE_EQUAL( success(), stake( "actc", "alice1111111", core_sym::from_string("200.0000"), core_sym::from_string("100.0000") ) );
   ...
   BOOST_REQUIRE_EQUAL( false, get_row_by_account( N(gls.token), N(alice1111111), N(accounts), symbol{CORE_SYM}.to_symbol_code() ).empty() );

   //remove row
   base_tester::push_action( N(gls.token), N(close), N(alice1111111), mvo()
                             ( "owner", "alice1111111" )
                             ( "symbol", symbol{CORE_SYM} )
   );
   BOOST_REQUIRE_EQUAL( true, get_row_by_account( N(gls.token), N(alice1111111), N(accounts), symbol{CORE_SYM}.to_symbol_code() ).empty() );

   auto rlm = control->get_resource_limits_manager();
   auto actcram_ram_usage = rlm.get_account_ram_usage(N(gls.ram));
   auto alice_ram_usage = rlm.get_account_ram_usage(N(alice1111111));

   BOOST_REQUIRE_EQUAL( success(), sellram( "alice1111111", 2048 ) );

   //make sure that ram was billed to alice, not to gls.ram
   BOOST_REQUIRE_EQUAL( true, alice_ram_usage < rlm.get_account_ram_usage(N(alice1111111)) );
   BOOST_REQUIRE_EQUAL( actcram_ram_usage, rlm.get_account_ram_usage(N(gls.ram)) );

} FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE( ram_gift, actc_system_tester ) try {
   ...
   */
   ...
   BOOST_REQUIRE_EQUAL( success(), unstake( "actc", "alice1111111", core_sym::from_string("20.0000"), core_sym::from_string("10.0000") ) );
...
BOOST_FIXTURE_TEST_CASE( rex_auth, actc_system_tester ) try {
   ...
   const asset one_actc      = core_sym::from_string("1.0000");
   ...
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(deposit), mvo()("owner", alice)("amount", one_actc) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(withdraw), mvo()("owner", alice)("amount", one_actc) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(buyrex), mvo()("from", alice)("amount", one_actc) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg),
                        push_action( bob, N(unstaketorex), mvo()("owner", alice)("receiver", alice)("from_net", one_actc)("from_cpu", one_actc) ) );
   ...
   BOOST_REQUIRE_EQUAL( error(error_msg),
                        push_action( bob, N(rentcpu), mvo()("from", alice)("receiver", alice)("loan_payment", one_actc)("loan_fund", one_actc) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg),
                        push_action( bob, N(rentnet), mvo()("from", alice)("receiver", alice)("loan_payment", one_actc)("loan_fund", one_actc) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(fundcpuloan), mvo()("from", alice)("loan_num", 1)("payment", one_actc) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(fundnetloan), mvo()("from", alice)("loan_num", 1)("payment", one_actc) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(defcpuloan), mvo()("from", alice)("loan_num", 1)("amount", one_actc) ) );
   BOOST_REQUIRE_EQUAL( error(error_msg), push_action( bob, N(defnetloan), mvo()("from", alice)("loan_num", 1)("amount", one_actc) ) );
   ...
   BOOST_REQUIRE_EQUAL( error("missing authority of actc"), push_action( alice, N(setrex), mvo()("balance", one_actc) ) );

} FC_LOG_AND_RETHROW()


BOOST_FIXTURE_TEST_CASE( buy_sell_rex, actc_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( buy_sell_small_rex, actc_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( unstake_buy_rex, actc_system_tester, * boost::unit_test::tolerance(1e-10) ) try {
...
//                           vote( alice, std::vector<account_name>(producer_names.begin(), producer_names.begin() + 21) ) );
      const asset init_actc_stake_balance = get_balance( N(gls.stake) );
      ...
      BOOST_REQUIRE_EQUAL( tot_stake,                               get_balance( N(gls.rex) ) );
      BOOST_REQUIRE_EQUAL( tot_stake,                               init_actc_stake_balance - get_balance( N(gls.stake) ) );
...
BOOST_FIXTURE_TEST_CASE( buy_rent_rex, actc_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( buy_sell_sell_rex, actc_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( buy_sell_claim_rex, actc_system_tester ) try {
   ...
   const int64_t init_alice_rex_stake = ( actc::chain::uint128_t(init_alice_rex.get_amount()) * total_lendable ) / total_rex;
...
BOOST_FIXTURE_TEST_CASE( rex_loans, actc_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( rex_loan_checks, actc_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( ramfee_namebid_to_rex, actc_system_tester ) try {
   ...
   asset cur_ramfee_balance = get_balance( N(gls.ramfee) );
   BOOST_REQUIRE_EQUAL( success(),                      buyram( alice, alice, core_sym::from_string("20.0000") ) );
   BOOST_REQUIRE_EQUAL( get_balance( N(gls.ramfee) ), core_sym::from_string("0.1000") + cur_ramfee_balance );
   ...
   cur_ramfee_balance = get_balance( N(gls.ramfee) );
   asset cur_rex_balance = get_balance( N(gls.rex) );
   ...
   BOOST_REQUIRE_EQUAL( cur_ramfee_balance,                get_balance( N(gls.ramfee) ) );
   BOOST_REQUIRE_EQUAL( get_balance( N(gls.rex) ),       cur_rex_balance + core_sym::from_string("0.3500") );

   cur_rex_balance = get_balance( N(gls.rex) );
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("23.7000"), get_balance( N(gls.names) ) );
   BOOST_REQUIRE_EQUAL( success(),                        bidname( alice, N(rndmbid), core_sym::from_string("29.3500") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("29.3500"), get_balance( N(gls.names) ));
   ...
   BOOST_REQUIRE_EQUAL( get_balance( N(gls.rex) ),      cur_rex_balance + core_sym::from_string("34.3500") );
   BOOST_REQUIRE_EQUAL( 0,                                get_balance( N(gls.names) ).get_amount() );
...
BOOST_FIXTURE_TEST_CASE( rex_maturity, actc_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( rex_savings, actc_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( update_rex, actc_system_tester, * boost::unit_test::tolerance(1e-10) ) try {
...
//                        vote( alice, std::vector<account_name>(producer_names.begin(), producer_names.begin() + 21) ) );
   ...
   const int64_t init_alice_rex_stake = ( actc::chain::uint128_t(init_rex.get_amount()) * total_lendable ) / total_rex;
...
BOOST_FIXTURE_TEST_CASE( update_rex_vote, actc_system_tester, * boost::unit_test::tolerance(1e-8) ) try {
...
BOOST_FIXTURE_TEST_CASE( deposit_rex_fund, actc_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( rex_lower_bound, actc_system_tester ) try {
   ...
   double rex_per_actc = double(tot_rex) / double(tot_lendable);
   int64_t sell_amount = rex_per_actc * ( tot_unlent - 0.09 * tot_lent );
   produce_block( fc::days(5) );
   BOOST_REQUIRE_EQUAL( success(), sellrex( alice, asset( sell_amount, rex_sym ) ) );
   BOOST_REQUIRE_EQUAL( success(), cancelrexorder( alice ) );
   sell_amount = rex_per_actc * ( tot_unlent - 0.1 * tot_lent );
   BOOST_REQUIRE_EQUAL( success(), sellrex( alice, asset( sell_amount, rex_sym ) ) );
...
BOOST_FIXTURE_TEST_CASE( close_rex, actc_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( set_rex, actc_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( b1_vesting, actc_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( rex_return, actc_system_tester ) try {
...
BOOST_AUTO_TEST_CASE( setabi_bios ) try {
   ...
   t.create_account(N(gls.token));
   t.set_abi( N(gls.token), contracts::token_abi().data() );
   {
      auto res = t.get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(gls.token) );
      _abi_hash abi_hash;
   ...
   t.set_abi( N(gls.token), contracts::system_abi().data() );
   {
      auto res = t.get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(gls.token) );
      _abi_hash abi_hash;
      auto abi_hash_var = abi_ser.binary_to_variant( "abi_hash", res, base_tester::abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( setabi, actc_system_tester ) try {
   set_abi( N(gls.token), contracts::token_abi().data() );
   {
      auto res = get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(gls.token) );
      _abi_hash abi_hash;
   ...
   set_abi( N(gls.token), contracts::system_abi().data() );
   {
      auto res = get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(gls.token) );
      _abi_hash abi_hash;
      auto abi_hash_var = abi_ser.binary_to_variant( "abi_hash", res, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( change_limited_account_back_to_unlimited, actc_system_tester ) try {
   BOOST_REQUIRE( get_total_stake( "actc" ).is_null() );

   transfer( N(actc), N(alice1111111), core_sym::from_string("1.0000") );

   auto error_msg = stake( N(alice1111111), N(actc), core_sym::from_string("0.0000"), core_sym::from_string("1.0000") );
   auto semicolon_pos = error_msg.find(';');

   BOOST_REQUIRE_EQUAL( error("account actc has insufficient ram"),
   ...
   push_action( N(actc), N(setalimits), mvo()
                                          ("account", "actc")
   ...
   stake( N(alice1111111), N(actc), core_sym::from_string("0.0000"), core_sym::from_string("1.0000") );

   REQUIRE_MATCHING_OBJECT( get_total_stake( "actc" ), mvo()
      ("owner", "actc")
   ...
   BOOST_REQUIRE_EQUAL( success(),
                        push_action( N(actc), N(setacctnet), mvo()
                           ("account", "actc")
                           ("net_weight", -1)
                        )
   );
   ...
   BOOST_REQUIRE_EQUAL( success(),
                        push_action( N(actc), N(setacctcpu), mvo()
                           ("account", "actc")
                           ("cpu_weight", -1)
...
BOOST_FIXTURE_TEST_CASE( buy_pin_sell_ram, actc_system_tester ) try {
   BOOST_REQUIRE( get_total_stake( "actc" ).is_null() );

   transfer( N(actc), N(alice1111111), core_sym::from_string("1020.0000") );

   auto error_msg = stake( N(alice1111111), N(actc), core_sym::from_string("10.0000"), core_sym::from_string("10.0000") );
   auto semicolon_pos = error_msg.find(';');
   ...
   BOOST_REQUIRE_EQUAL( success(), buyrambytes( N(alice1111111), N(actc), static_cast<uint32_t>(ram_bytes_needed) ) );

   auto tokens_paid_for_ram = alice_original_balance - get_balance( N(alice1111111) );

   auto total_res = get_total_stake( "actc" );
   ...
   BOOST_REQUIRE_EQUAL( success(),
                        push_action( N(actc), N(setacctram), mvo()
                           ("account", "actc")
                           ("ram_bytes", total_res["ram_bytes"].as_int64() )
                        )
   );

   auto actc_original_balance = get_balance( N(actc) );

   BOOST_REQUIRE_EQUAL( success(), sellram( N(actc), total_res["ram_bytes"].as_int64() ) );

   auto tokens_received_by_selling_ram = get_balance( N(actc) ) - actc_original_balance;
