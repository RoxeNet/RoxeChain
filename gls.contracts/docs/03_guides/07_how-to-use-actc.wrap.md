...
First, the account `gls.wrap` needs to be created. Since it has the restricted `actc.` prefix, only a privileged account can create this account. So this guide will use the `actc` account to create the `gls.wrap` account. On typical live blockchain configurations, the `actc` account can only be controlled by a supermajority of the current active block producers. So, this guide will use the `actc.msig` contract to help coordinate the approvals of the proposed transaction that creates the `gls.wrap` account.
...
Three unsigned transactions will be generated using clactc and then the actions within those transactions will be appropriately stitched together into a single transaction which will later be proposed using the actc.msig contract.
...
Next, the action JSONs of the previously generated transactions will be used to construct a unified transaction which will eventually be proposed with the actc.msig contract. A good way to get started is to make a copy of the generated_newaccount_trx.json file (call the copied file create_wrap_account_trx.json) and edit the first three fields so it looks something like the following:
```
...
It will be useful to have a JSON of the active permissions of each of the active block producers for later when proposing transactions using the actc.msig contract.
...
#    actc.msig <= actc.msig::propose          {"proposer":"blkproducera","proposal_name":"createwrap","requested":[{"actor":"blkproducera","permis...
...
#    actc.msig <= actc.msig::approve          {"proposer":"blkproducera","proposal_name":"createwrap","level":{"actor":"blkproducerb","permission"...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 1.1.4 Execute the transaction to create the gls.wrap account

When the necessary approvals are collected (in this example, with 21 block producers, at least 15 of their approvals were required), anyone can push the `actc.msig::exec` action which executes the approved transaction. It makes a lot of sense for the lead block producer who proposed the transaction to also execute it (this will incur another temporary RAM cost for the deferred transaction that is generated by the actc.msig contract).
...
#    actc.msig <= actc.msig::exec             {"proposer":"blkproducera","proposal_name":"createwrap","executer":"blkproducera"}
warning: transaction executed locally, but may not be confirmed by the network yet
```
...
#    actc.msig <= actc.msig::propose          {"proposer":"blkproducera","proposal_name":"deploywrap","requested":[{"actor":"blkproducera","permis...
warning: transaction executed locally, but may not be confirmed by the network yet
```
...
#    actc.msig <= actc.msig::approve          {"proposer":"blkproducera","proposal_name":"deploywrap","level":{"actor":"blkproducerb","permission"...
warning: transaction executed locally, but may not be confirmed by the network yet
```
...
#    actc.msig <= actc.msig::exec             {"proposer":"blkproducera","proposal_name":"deploywrap","executer":"blkproducera"}
```
...
This example will demonstrate how to use the deployed gls.wrap contract together with the actc.msig contract to allow a greater than two-thirds supermajority of block producers of an ACTC blockchain to change the owner authority of an arbitrary account. The example will use clactc: in particular, the `clactc multisig` command, the `clactc set account permission` sub-command, and the `clactc wrap exec` sub-command. However, the guide also demonstrates what to do if the `clactc wrap exec` sub-command is not available.
...
#    actc.msig <= actc.msig::propose          {"proposer":"blkproducera","proposal_name":"updatealice","requested":[{"actor":"blkproducera","permi...
warning: transaction executed locally, but may not be confirmed by the network yet
```
...
#    actc.msig <= actc.msig::approve          {"proposer":"blkproducera","proposal_name":"updatealice","level":{"actor":"blkproducerb","permission...
warning: transaction executed locally, but may not be confirmed by the network yet
```
...
#    actc.msig <= actc.msig::exec             {"proposer":"blkproducera","proposal_name":"updatealice","executer":"blkproducera"}
warning: transaction executed locally, but may not be confirmed by the network yet
```
