...
#include <actclib/permission.hpp>
...
void multisig::propose() {
   ...
   actc_assert( trx_header.expiration >= actc::time_point_sec(now()), "transaction expired" );
   //actc_assert( trx_header.actions.size() > 0, "transaction must have at least one action" );

   proposals proptable( _self, proposer );
   actc_assert( proptable.find( proposal_name ) == proptable.end(), "proposal with the same name exists" );
   ...
   actc_assert( res > 0, "transaction authorization failed" );
...
void multisig::approve( account_name proposer, name proposal_name, permission_level level ) {
   ...
   actc_assert( itr != apps.requested_approvals.end(), "approval is not on the list of requested approvals" );
...
void multisig::unapprove( account_name proposer, name proposal_name, permission_level level ) {
   ...
   actc_assert( itr != apps.provided_approvals.end(), "no approval previously granted" );
...
void multisig::cancel( account_name proposer, name proposal_name, account_name canceler ) {
   ...
   if( canceler != proposer ) {
      actc_assert( unpack<transaction_header>( prop.packed_transaction ).expiration < actc::time_point_sec(now()), "cannot cancel until expiration" );
   }
...
actc_ABI( actc::multisig, (propose)(approve)(unapprove)(cancel)(exec) )
