...
#define N(X) ::actc::string_to_name(#X)
namespace actc {
...
// Helper macro for actc_API
#define actc_API_CALL( r, OP, elem ) \
   case ::actc::string_to_name( BOOST_PP_STRINGIZE(elem) ): \
      actc::execute_action( &thiscontract, &OP::elem ); \
      break;

// Helper macro for actc_ABI
#define actc_API( TYPE,  MEMBERS ) \
   BOOST_PP_SEQ_FOR_EACH( actc_API_CALL, TYPE, MEMBERS )
 ...
 * To be able to use this macro, the contract needs to be derived from actc::contract
 * 
 ...
 * Example:
 * @code
 * actc_ABI( actc::bios, (setpriv)(setalimits)(setglimits)(setprods)(reqauth) )
 * @endcode
 */
#define actc_ABI( TYPE, MEMBERS ) \
extern "C" { \
   void apply( uint64_t receiver, uint64_t code, uint64_t action ) { \
      auto self = receiver; \
      if( action == N(onerror)) { \
         /* onerror is only valid if it is for the "actc" code account and authorized by "actc"'s "active permission */ \
         actc_assert(code == N(actc), "onerror action's are only valid from the \"actc\" system account"); \
      } \
      if( code == self || action == N(onerror) ) { \
         TYPE thiscontract( self ); \
         switch( action ) { \
            actc_API( TYPE, MEMBERS ) \
         } \
