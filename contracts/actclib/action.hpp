...
namespace actc {
   ...
   struct action {
      ...
      void send_context_free() const {
         actc_assert( authorization.size() == 0, "context free actions cannot have authorizations");
         auto serialize = pack(*this);
      ...
      template<typename T>
      T data_as() {
         actc_assert( name == T::get_name(), "Invalid name" );
         actc_assert( account == T::get_account(), "Invalid account" );
...
INLINE_ACTION_SENDER3( CONTRACT_CLASS, NAME, ::actc::string_to_name(#NAME) )
 ...
 * @param NAME - The name of the action
 */
#define ACTION( CODE, NAME ) struct NAME : ::actc::action_meta<CODE, ::actc::string_to_name(#NAME) >
