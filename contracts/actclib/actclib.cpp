...
namespace actc {
   ...
   void set_blockchain_parameters(const actc::blockchain_parameters& params) {
      char buf[sizeof(actc::blockchain_parameters)];
      actc::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(actc::blockchain_parameters& params) {
      char buf[sizeof(actc::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      actc_assert( size <= sizeof(buf), "buffer is too small" );
      actc::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
   ...
   private:
      ...
      public:
         ...
         {
            actc_assert(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return actc::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = actc::memory_heap.malloc(count*size);
...
{
   return actc::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return actc::memory_heap.free(ptr);
}
