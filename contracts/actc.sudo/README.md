...
Execute a transaction while bypassing regular authorization checks (requires authorization of actc.sudo which needs to be a privileged account).

### actc.sudo::exec    executer trx
...
## 2. Installing the actc.sudo contract

The actc.sudo contract needs to be installed on a privileged account to function. It is recommended to use the account `actc.sudo`.

First, the account `actc.sudo` needs to be created. Since it has the restricted `actc.` prefix, only a privileged account can create this account. So this guide will use the `actc` account to create the `actc.sudo` account. On typical live blockchain configurations, the `actc` account can only be controlled by a supermajority of the current active block producers. So, this guide will use the `actc.msig` contract to help coordinate the approvals of the proposed transaction that creates the `actc.sudo` account.

The `actc.sudo` account also needs to have sufficient RAM to host the contract and sufficient CPU and network bandwidth to deploy the contract. This means that the creator of the account (`actc`) needs to gift sufficient RAM to the new account and delegate (preferably with transfer) sufficient bandwidth to the new account. To pull this off the `actc` account needs to have enough of the core system token (the `SYS` token will be used within this guide) in its liquid balance. So prior to continuing with the next steps of this guide, the active block producers of the chain who are coordinating this process need to ensure that a sufficient amount of core system tokens that they are authorized to spend is placed in the liquid balance of the `actc` account.

This guide will be using clactc to carry out the process.

### 2.1 Create the actc.sudo account

#### 2.1.1 Generate the transaction to create the actc.sudo account

The transaction to create the `actc.sudo` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the clactc command to propose the transaction to the actc.msig contract.
...
```
$ clactc system newaccount -s -j -d --transfer --stake-net "1.000 SYS" --stake-cpu "1.000 SYS" --buy-ram-kbytes 50 actc actc.sudo actc8MMUW11TAdTDxqdSwSqJodefSoZbFhcprndomgLi9MeR2o8MT4 > generated_account_creation_trx.json
726964ms thread-0   main.cpp:429                  create_action        ] result: {"binargs":"0000000000ea305500004d1a03ea305500c80000"} arg: {"code":"actc","action":"buyrambytes","args":{"payer":"actc","receiver":"actc.sudo","bytes":51200}}
726967ms thread-0   main.cpp:429                  create_action        ] result: {"binargs":"0000000000ea305500004d1a03ea3055102700000000000004535953000000001027000000000000045359530000000001"} arg: {"code":"actc","action":"delegatebw","args":{"from":"actc","receiver":"actc.sudo","stake_net_quantity":"1.0000 SYS","stake_cpu_quantity":"1.0000 SYS","transfer":true}}
...
Fourth, generate a transaction containing the `actc::setpriv` action which will make the `actc.sudo` account privileged:
```
$ clactc push action -s -j -d actc setpriv '{"account": "actc.sudo", "is_priv": 1}' -p actc > generated_setpriv_trx.json
...
#### 2.1.2 Propose the transaction to create the actc.sudo account
...
```
$ clactc multisig propose_trx createsudo producer_permissions.json create_sudo_account_trx.json blkproducera
executed transaction: bf6aaa06b40e2a35491525cb11431efd2b5ac94e4a7a9c693c5bf0cfed942393  744 bytes  772 us
#    actc.msig <= actc.msig::propose          {"proposer":"blkproducera","proposal_name":"createsudo","requested":[{"actor":"blkproducera","permis...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.1.3 Review and approve the transaction to create the actc.sudo account
...
```
$ clactc multisig review blkproducera createsudo > create_sudo_account_trx_to_review.json
...
```
$ clactc multisig propose_trx -j -s -d createsudo '[]' create_sudo_account_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_create_sudo_trx_serialized.hex
...
```
$ clactc multisig approve blkproducera createsudo '{"actor": "blkproducerb", "permission": "active"}' -p blkproducerb
executed transaction: 03a907e2a3192aac0cd040c73db8273c9da7696dc7960de22b1a479ae5ee9f23  128 bytes  472 us
#    actc.msig <= actc.msig::approve          {"proposer":"blkproducera","proposal_name":"createsudo","level":{"actor":"blkproducerb","permission"...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.1.4 Execute the transaction to create the actc.sudo account
...
```
$ clactc multisig exec blkproducera createsudo blkproducera
executed transaction: 7ecc183b99915cc411f96dde7c35c3fe0df6e732507f272af3a039b706482e5a  160 bytes  850 us
#    actc.msig <= actc.msig::exec             {"proposer":"blkproducera","proposal_name":"createsudo","executer":"blkproducera"}
warning: transaction executed locally, but may not be confirmed by the network yet
```

Anyone can now verify that the `actc.sudo` was created:
```
$ clactc get account actc.sudo
privileged: true
permissions:
...
### 2.2 Deploy the actc.sudo contract

#### 2.2.1  Generate the transaction to deploy the actc.sudo contract

The transaction to deploy the contract to the `actc.sudo` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the clactc command to propose the transaction to the actc.msig contract.

The easy way to generate this transaction is using clactc:
```
$ clactc set contract -s -j -d actc.sudo contracts/actc.sudo/ > deploy_sudo_contract_trx.json
Reading WAST/WASM from contracts/actc.sudo/actc.sudo.wasm...
...
#### 2.2.2 Propose the transaction to deploy the actc.sudo contract
...
```
$ clactc multisig propose_trx deploysudo producer_permissions.json deploy_sudo_contract_trx.json blkproducera
executed transaction: 9e50dd40eba25583a657ee8114986a921d413b917002c8fb2d02e2d670f720a8  4312 bytes  871 us
#    actc.msig <= actc.msig::propose          {"proposer":"blkproducera","proposal_name":"deploysudo","requested":[{"actor":"blkproducera","permis...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.2.3 Review and approve the transaction to deploy the actc.sudo contract

Each of the potential approvers of the proposed transaction (i.e. the active block producers) should first review the proposed transaction to make sure they are not approving anything that they do not agree to.

The proposed transaction can be reviewed using the `clactc multisig review` command:
```
$ clactc multisig review blkproducera deploysudo > deploy_sudo_contract_trx_to_review.json
...
```
$ clactc multisig propose_trx -j -s -d deploysudo '[]' deploy_sudo_contract_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_deploy_sudo_trx_serialized.hex
...
When an approver (e.g. `blkproducerb`) is satisfied with the proposed transaction, they can simply approve it:
```
$ clactc multisig approve blkproducera deploysudo '{"actor": "blkproducerb", "permission": "active"}' -p blkproducerb
executed transaction: d1e424e05ee4d96eb079fcd5190dd0bf35eca8c27dd7231b59df8e464881abfd  128 bytes  483 us
#    actc.msig <= actc.msig::approve          {"proposer":"blkproducera","proposal_name":"deploysudo","level":{"actor":"blkproducerb","permission"...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.2.4 Execute the transaction to create the actc.sudo account

When the necessary approvals are collected (in this example, with 21 block producers, at least 15 of their approvals were required), anyone can push the `actc.msig::exec` action which executes the approved transaction. It makes a lot of sense for the lead block producer who proposed the transaction to also execute it (this will incur another temporary RAM cost for the deferred transaction that is generated by the actc.msig contract).

```
$ clactc multisig exec blkproducera deploysudo blkproducera
executed transaction: e8da14c6f1fdc3255b5413aactcfd0d89b18f832a4cc18c4324ea2beec6abd483  160 bytes  1877 us
#    actc.msig <= actc.msig::exec             {"proposer":"blkproducera","proposal_name":"deploysudo","executer":"blkproducera"}
```

Anyone can now verify that the `actc.sudo` contract was deployed correctly.

```
$ clactc get code -a retrieved-actc.sudo.abi actc.sudo
code hash: 1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c
saving abi to retrieved-actc.sudo.abi
$ sha256sum contracts/actc.sudo/actc.sudo.wasm
1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c  contracts/actc.sudo/actc.sudo.wasm
```

If the two hashes match then the local WebAssembly code is the one deployed on the blockchain. The retrieved ABI, which was stored in the file retrieved-actc.sudo.abi, can then be compared to the original ABI of the contract (contracts/actc.sudo/actc.sudo.abi) to ensure they are semantically the same.

## 3. Using the actc.sudo contract

### 3.1 Example: Updating owner authority of an arbitrary account

This example will demonstrate how to use the deployed actc.sudo contract together with the actc.msig contract to allow a greater than two-thirds supermajority of block producers of an actc blockchain to change the owner authority of an arbitrary account. The example will use clactc: in particular, the `clactc multisig` command, the `clactc set account permission` sub-command, and the `clactc sudo exec` sub-command. However, the guide also demonstrates what to do if the `clactc sudo exec` sub-command is not available.
...
The next step is to generate the transaction containing the `actc.sudo::exec` action. This action will contain the transaction in update_alice_owner_trx.json as part of its action payload data.

```
$ clactc sudo exec -s -j -d blkproducera update_alice_owner_trx.json > sudo_update_alice_owner_trx.json
```
...
If the `clactc sudo` command is not available, there is an alternative way to generate the above transaction. There is no need to continue reading the remaining of sub-section 3.1.1 if the sudo_update_alice_owner_trx.json file was already generated with content similar to the above using the `clactc sudo exec` sub-command method.
...
Then generate the template for the transaction containing the `actc.sudo::exec` action:
```
$ clactc push action -s -j -d actc.sudo exec '{"executer": "blkproducera", "trx": ""}' > sudo_update_alice_owner_trx.json
$ cat sudo_update_alice_owner_trx.json
{
...
```
$ clactc multisig propose_trx updatealice producer_permissions.json sudo_update_alice_owner_trx.json blkproducera
...
Each of the potential approvers of the proposed transaction (i.e. the active block producers) should first review the proposed transaction to make sure they are not approving anything that they do not agree to.
```
$ clactc multisig review blkproducera updatealice > sudo_update_alice_owner_trx_to_review.json
...
```
$ clactc multisig propose_trx -j -s -d updatealice '[]' sudo_update_alice_owner_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_sudo_update_alice_owner_trx_serialized.hex
