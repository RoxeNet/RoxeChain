...
namespace dcciosystem {
   using dccio::asset;
   using dccio::indexed_by;
   using dccio::const_mem_fun;
   using dccio::bytes;
   using dccio::print;
   using dccio::permission_level;
   using std::map;
   using std::pair;
   ...
   struct user_resources {
      ...
      dccLIB_SERIALIZE( user_resources, (owner)(net_weight)(cpu_weight)(ram_bytes) )
   };
   ...
   struct delegated_bandwidth {
      ...
      // explicit serialization macro is not necessary, used here only to improve compilation time
      dccLIB_SERIALIZE( delegated_bandwidth, (from)(to)(net_weight)(cpu_weight) )
   ...
   struct refund_request {
      account_name  owner;
      time          request_time;
      dccio::asset  net_amount;
      dccio::asset  cpu_amount;

      uint64_t  primary_key()const { return owner; }

      // explicit serialization macro is not necessary, used here only to improve compilation time
      dccLIB_SERIALIZE( refund_request, (owner)(request_time)(net_amount)(cpu_amount) )
   };
    ...
    */
   typedef dccio::multi_index< N(userres), user_resources>      user_resources_table;
   typedef dccio::multi_index< N(delband), delegated_bandwidth> del_bandwidth_table;
   typedef dccio::multi_index< N(refunds), refund_request>      refunds_table;
   ...
   void system_contract::buyrambytes( account_name payer, account_name receiver, uint32_t bytes ) {
      ...
      auto tmp = *itr;
      auto dccout = tmp.convert( asset(bytes,S(0,RAM)), CORE_SYMBOL );

      buyram( payer, receiver, dccout );
   }
   ...
   {
      require_auth( payer );
      dccio_assert( quant.amount > 0, "must purchase a positive amount" );
      ...
      INLINE_ACTION_SENDER(dccio::token, transfer)( N(dccio.token), {payer,N(active)},
         { payer, N(dccio.ram), quant_after_fee, std::string("buy ram") } );

      if( fee.amount > 0 ) {
         INLINE_ACTION_SENDER(dccio::token, transfer)( N(dccio.token), {payer,N(active)},
                                                       { payer, N(dccio.ramfee), fee, std::string("ram fee") } );
      }
      ...
      dccio_assert( bytes_out > 0, "must reserve a positive amount" );
   ...
   void system_contract::sellram( account_name account, int64_t bytes ) {
      ...
      dccio_assert( bytes > 0, "cannot sell negative byte" );

      user_resources_table  userres( _self, account );
      auto res_itr = userres.find( account );
      dccio_assert( res_itr != userres.end(), "no resource row" );
      dccio_assert( res_itr->ram_bytes >= bytes, "insufficient quota" );
      ...
      dccio_assert( tokens_out.amount > 1, "token amount received from selling ram is too low" );
      ...
      dccio_assert( _gstate.total_ram_stake >= 0, "error, attempt to unstake more tokens than previously staked" );
      ...
      INLINE_ACTION_SENDER(dccio::token, transfer)( N(dccio.token), {N(dccio.ram),N(active)},
                                                       { N(dccio.ram), account, asset(tokens_out), std::string("sell ram") } );

      auto fee = ( tokens_out.amount + 199 ) / 200; /// .5% fee (round up)
      // since tokens_out.amount was asserted to be at least 2 earlier, fee.amount < tokens_out.amount
      
      if( fee > 0 ) {
         INLINE_ACTION_SENDER(dccio::token, transfer)( N(dccio.token), {account,N(active)},
            { account, N(dccio.ramfee), asset(fee), std::string("sell ram fee") } );
      }
   }
   ...
   void validate_b1_vesting( int64_t stake ) {
      ...
      dccio_assert( max_claimable - claimable <= stake, "b1 can only claim their tokens over 10 years" );
   }
   ...
   {
      require_auth( from );
      dccio_assert( stake_net_delta != asset(0) || stake_cpu_delta != asset(0), "should stake non-zero amount" );
      dccio_assert( std::abs( (stake_net_delta + stake_cpu_delta).amount )
      ...
      // update stake delegated from "from" to "receiver"
      {
         ...
         dccio_assert( asset(0) <= itr->net_weight, "insufficient staked net bandwidth" );
         dccio_assert( asset(0) <= itr->cpu_weight, "insufficient staked cpu bandwidth" );
      ...
      // update totals of "receiver"
      {
         ...
         dccio_assert( asset(0) <= tot_itr->net_weight, "insufficient staked total net bandwidth" );
         dccio_assert( asset(0) <= tot_itr->cpu_weight, "insufficient staked total cpu bandwidth" );
      ...
      if ( N(dccio.stake) != source_stake_from ) { //for dccio both transfer and refund make no sense
         ...
         if( is_delegating_to_self || is_undelegating ) {
            if ( req != refunds_tbl.end() ) { //need to update refund
               ...
               dccio_assert( asset(0) <= req->net_amount, "negative net refund amount" ); //should never happen
               dccio_assert( asset(0) <= req->cpu_amount, "negative cpu refund amount" ); //should never happen
         ...
         if ( asset(0) < transfer_amount ) {
            INLINE_ACTION_SENDER(dccio::token, transfer)( N(dccio.token), {source_stake_from, N(active)},
               { source_stake_from, N(dccio.stake), asset(transfer_amount), std::string("stake bandwidth") } );
         }
      }
      ...
      // update voting power
      {
         ...
         dccio_assert( 0 <= from_voter->staked, "stake for voting cannot be negative");
         if( from == N(b1) ) {
   ...
   {
      dccio_assert( stake_cpu_quantity >= asset(0), "must stake a positive amount" );
      dccio_assert( stake_net_quantity >= asset(0), "must stake a positive amount" );
      dccio_assert( stake_net_quantity + stake_cpu_quantity > asset(0), "must stake a positive amount" );
      dccio_assert( !transfer || from != receiver, "cannot use transfer flag if delegating to self" );
   ...
   {
      dccio_assert( asset() <= unstake_cpu_quantity, "must unstake a positive amount" );
      dccio_assert( asset() <= unstake_net_quantity, "must unstake a positive amount" );
      dccio_assert( asset() < unstake_cpu_quantity + unstake_net_quantity, "must unstake a positive amount" );
      dccio_assert( _gstate.total_activated_stake >= min_activated_stake,
   ...
   void system_contract::refund( const account_name owner ) {
      ...
      dccio_assert( req != refunds_tbl.end(), "refund request not found" );
      dccio_assert( req->request_time + refund_delay <= now(), "refund is not available yet" );
      // Until now() becomes NOW, the fact that now() is the timestamp of the previous block could in theory
      // allow people to get their tokens earlier than the 3 day delay if the unstake happened immediately after many
      // consecutive missed blocks.

      INLINE_ACTION_SENDER(dccio::token, transfer)( N(dccio.token), {N(dccio.stake),N(active)},
                                                    { N(dccio.stake), req->owner, req->net_amount + req->cpu_amount, std::string("unstake") } );
