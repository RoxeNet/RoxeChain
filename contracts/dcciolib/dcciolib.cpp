...
namespace dccio {
   ...
   void set_blockchain_parameters(const dccio::blockchain_parameters& params) {
      char buf[sizeof(dccio::blockchain_parameters)];
      dccio::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(dccio::blockchain_parameters& params) {
      char buf[sizeof(dccio::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      dccio_assert( size <= sizeof(buf), "buffer is too small" );
      dccio::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
   ...
   private:
      ...
      public:
         ...
         {
            dccio_assert(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return dccio::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = dccio::memory_heap.malloc(count*size);
...
{
   return dccio::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return dccio::memory_heap.free(ptr);
}
