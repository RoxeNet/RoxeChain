...
namespace dccio {
    ...
    *    int  c; //4
    *
    *    dccLIB_SERIALIZE( dummy_action, (a)(b)(c) )
    *  };
   ...
   struct permission_level {
      ...
      dccLIB_SERIALIZE( permission_level, (actor)(permission) )
   };
   ...
   struct action {
      ...
      dccLIB_SERIALIZE( action, (account)(name)(authorization)(data) )
      ...
      void send_context_free() const {
         dccio_assert( authorization.size() == 0, "context free actions cannot have authorizations");
         auto serialize = pack(*this);
      ...
      template<typename T>
      T data_as() {
         dccio_assert( name == T::get_name(), "Invalid name" );
         dccio_assert( account == T::get_account(), "Invalid account" );
...
::dccio::inline_dispatcher<decltype(&CONTRACT_CLASS::FUNCTION_NAME), ACTION_NAME>::call

#define INLINE_ACTION_SENDER2( CONTRACT_CLASS, NAME )\
INLINE_ACTION_SENDER3( CONTRACT_CLASS, NAME, ::dccio::string_to_name(#NAME) )
 ...
 * @param NAME - The name of the action
 */
#define ACTION( CODE, NAME ) struct NAME : ::dccio::action_meta<CODE, ::dccio::string_to_name(#NAME) >
