...
#define N(X) ::dccio::string_to_name(#X)
namespace dccio {
    ...
    * ```
    *
    * For this to work the Actions must be derived from dccio::contract
    *
    */
   ...
   bool dispatch( uint64_t code, uint64_t act ) {
      ...
      return dccio::dispatch<Contract,SecondAction,Actions...>( code, act );
   }
    ...
    * @tparam T - The contract class that has the correponding action handler, this contract should be derived from dccio::contract
...
// Helper macro for dccIO_API
#define dccIO_API_CALL( r, OP, elem ) \
   case ::dccio::string_to_name( BOOST_PP_STRINGIZE(elem) ): \
      dccio::execute_action( &thiscontract, &OP::elem ); \
      break;

// Helper macro for dccIO_ABI
#define dccIO_API( TYPE,  MEMBERS ) \
   BOOST_PP_SEQ_FOR_EACH( dccIO_API_CALL, TYPE, MEMBERS )
 ...
 * To be able to use this macro, the contract needs to be derived from dccio::contract
 * 
 ...
 * Example:
 * @code
 * dccIO_ABI( dccio::bios, (setpriv)(setalimits)(setglimits)(setprods)(reqauth) )
 * @endcode
 */
#define dccIO_ABI( TYPE, MEMBERS ) \
extern "C" { \
   void apply( uint64_t receiver, uint64_t code, uint64_t action ) { \
      auto self = receiver; \
      if( action == N(onerror)) { \
         /* onerror is only valid if it is for the "dccio" code account and authorized by "dccio"'s "active permission */ \
         dccio_assert(code == N(dccio), "onerror action's are only valid from the \"dccio\" system account"); \
      } \
      if( code == self || action == N(onerror) ) { \
         TYPE thiscontract( self ); \
         switch( action ) { \
            dccIO_API( TYPE, MEMBERS ) \
         } \
         /* does not allow destructor of thiscontract to run: dccio_exit(0); */ \
      } \
   } \
} \
 /// @}  dispatcher
