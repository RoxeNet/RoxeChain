...
#include <roxe/wallet_plugin/yubihsm_wallet.hpp>
...
struct yubihsm_wallet_impl {
   ...
   public_key_type create() {
      ...
      try {
         if((rc = yh_util_generate_ec_key(session, &new_key_id, "kroxed created key", authkey_domains, &creation_caps, YH_ALGO_EC_P256)))
