...
public:
   ...
   void basic_setup() {
      ...
      create_accounts({ N(actc.token), N(actc.ram), N(actc.ramfee), N(actc.stake),
               N(actc.bpay), N(actc.vpay), N(actc.saving), N(actc.names), N(actc.rex) });


      produce_blocks( 100 );
      set_code( N(actc.token), contracts::token_wasm());
   ...
   void create_core_token( symbol core_symbol = symbol{CORE_SYM} ) {
      ...
      create_currency( N(actc.token), config::system_account_name, asset(100000000000000, core_symbol) );
   ...
   void remaining_setup() {
      ...
      BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"), get_balance("actc")  + get_balance("actc.ramfee") + get_balance("actc.stake") + get_balance("actc.ram") );
   }
   ...
   asset get_balance( const account_name& act, symbol balance_symbol = symbol{CORE_SYM} ) {
      vector<char> data = get_row_by_account( N(actc.token), act, N(accounts), balance_symbol.to_symbol_code().value );
   ...
   void issue_and_transfer( const name& to, const asset& amount, const name& manager = config::system_account_name ) {
      signed_transaction trx;
      trx.actions.emplace_back( get_action( N(actc.token), N(issue),
      ...
      if ( to != manager ) {
         trx.actions.emplace_back( get_action( N(actc.token), N(transfer),
                                               vector<permission_level>{{manager, config::active_name}},
                                               mutable_variant_object()
                                               ("from",     manager)
                                               ("to",       to )
   ...
   abi_serializer initialize_multisig() {
      abi_serializer msig_abi_ser;
      {
         ...
         BOOST_REQUIRE_EQUAL( success(), buyram( "actc", "actc.msig", core_sym::from_string("5000.0000") ) );
         produce_block();
