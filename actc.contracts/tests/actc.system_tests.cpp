...
#include "actc.system_tester.hpp"
struct _abi_hash {
   name owner;
   fc::sha256 hash;
};
...
BOOST_FIXTURE_TEST_CASE( buysell, actc_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( initial_ram_balance + core_sym::from_string("199.0000"), get_balance(N(actc.ram)) );
   BOOST_REQUIRE_EQUAL( initial_ramfee_balance + core_sym::from_string("1.0000"), get_balance(N(actc.ramfee)) );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake, actc_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( init_actc_stake_balance + core_sym::from_string("300.0000"), get_balance( N(actc.stake) ) );
...
//      BOOST_REQUIRE_EQUAL(success(), vote( N(producvotera), { N(defproducera),N(defproducerb),N(defproducerc) }));
      ...
      auto run_for_1year = [this](int64_t annual_rate, int64_t inflation_pay_factor, int64_t votepay_factor) {
         ...
         const int64_t final_savings = get_balance(N(actc.saving)).get_amount();
...
BOOST_AUTO_TEST_CASE(extreme_inflation) try {
   ...
   t.create_currency( N(actc.token), config::system_account_name, asset((1ll << 62) - 1, core_symbol) );
...
//   BOOST_REQUIRE_EQUAL(t.success(), t.vote( N(defproducera), { N(defproducera) }));
   ...
   {
      vector<char> data = t.get_row_by_account( N(actc.token), core_symbol.to_symbol_code().value, N(stat), core_symbol.to_symbol_code().value );
...
//   BOOST_REQUIRE_EQUAL( success(), vote( N(carl), { N(producer) } ) );
   ...
   // alice outbids bob on prefb
   {
      ...
      BOOST_REQUIRE_EQUAL( initial_names_balance + core_sym::from_string("0.1001"), get_balance(N(actc.names)) );
   }
...
BOOST_FIXTURE_TEST_CASE( actcram_ramusage, actc_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( false, get_row_by_account( N(actc.token), N(alice1111111), N(accounts), symbol{CORE_SYM}.to_symbol_code() ).empty() );

   //remove row
   base_tester::push_action( N(actc.token), N(close), N(alice1111111), mvo()
                             ( "owner", "alice1111111" )
                             ( "symbol", symbol{CORE_SYM} )
   );
   BOOST_REQUIRE_EQUAL( true, get_row_by_account( N(actc.token), N(alice1111111), N(accounts), symbol{CORE_SYM}.to_symbol_code() ).empty() );

   auto rlm = control->get_resource_limits_manager();
   auto actcram_ram_usage = rlm.get_account_ram_usage(N(actc.ram));
   auto alice_ram_usage = rlm.get_account_ram_usage(N(alice1111111));

   BOOST_REQUIRE_EQUAL( success(), sellram( "alice1111111", 2048 ) );

   //make sure that ram was billed to alice, not to actc.ram
   BOOST_REQUIRE_EQUAL( true, alice_ram_usage < rlm.get_account_ram_usage(N(alice1111111)) );
   BOOST_REQUIRE_EQUAL( actcram_ram_usage, rlm.get_account_ram_usage(N(actc.ram)) );
...
//                           vote( alice, std::vector<account_name>(producer_names.begin(), producer_names.begin() + 21) ) );
      ...
      BOOST_REQUIRE_EQUAL( tot_stake,                               get_balance( N(actc.rex) ) );
      BOOST_REQUIRE_EQUAL( tot_stake,                               init_actc_stake_balance - get_balance( N(actc.stake) ) );
...
BOOST_FIXTURE_TEST_CASE( ramfee_namebid_to_rex, actc_system_tester ) try {
   ...
   asset cur_ramfee_balance = get_balance( N(actc.ramfee) );
   BOOST_REQUIRE_EQUAL( success(),                      buyram( alice, alice, core_sym::from_string("20.0000") ) );
   BOOST_REQUIRE_EQUAL( get_balance( N(actc.ramfee) ), core_sym::from_string("0.1000") + cur_ramfee_balance );
   ...
   cur_ramfee_balance = get_balance( N(actc.ramfee) );
   asset cur_rex_balance = get_balance( N(actc.rex) );
   ...
   BOOST_REQUIRE_EQUAL( cur_ramfee_balance,                get_balance( N(actc.ramfee) ) );
   BOOST_REQUIRE_EQUAL( get_balance( N(actc.rex) ),       cur_rex_balance + core_sym::from_string("0.3500") );

   cur_rex_balance = get_balance( N(actc.rex) );
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("23.7000"), get_balance( N(actc.names) ) );
   BOOST_REQUIRE_EQUAL( success(),                        bidname( alice, N(rndmbid), core_sym::from_string("29.3500") ) );
   BOOST_REQUIRE_EQUAL( core_sym::from_string("29.3500"), get_balance( N(actc.names) ));
   ...
   BOOST_REQUIRE_EQUAL( get_balance( N(actc.rex) ),      cur_rex_balance + core_sym::from_string("34.3500") );
   BOOST_REQUIRE_EQUAL( 0,                                get_balance( N(actc.names) ).get_amount() );
...
BOOST_AUTO_TEST_CASE( setabi_bios ) try {
   ...
   t.create_account(N(actc.token));
   t.set_abi( N(actc.token), contracts::token_abi().data() );
   {
      auto res = t.get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(actc.token) );
      _abi_hash abi_hash;
   ...
   t.set_abi( N(actc.token), contracts::system_abi().data() );
   {
      auto res = t.get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(actc.token) );
      _abi_hash abi_hash;
      auto abi_hash_var = abi_ser.binary_to_variant( "abi_hash", res, base_tester::abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( setabi, actc_system_tester ) try {
   set_abi( N(actc.token), contracts::token_abi().data() );
   {
      auto res = get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(actc.token) );
      _abi_hash abi_hash;
   ...
   set_abi( N(actc.token), contracts::system_abi().data() );
   {
      auto res = get_row_by_account( config::system_account_name, config::system_account_name, N(abihash), N(actc.token) );
      _abi_hash abi_hash;
      auto abi_hash_var = abi_ser.binary_to_variant( "abi_hash", res, abi_serializer_max_time );
