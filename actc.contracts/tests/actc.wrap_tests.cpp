...
class actc_wrap_tester : public tester {
public:
   ...
   actc_wrap_tester() {
      ...
      set_code( N(actc.wrap), contracts::wrap_wasm(), &system_private_key );
      set_abi( N(actc.wrap), contracts::wrap_abi().data(), &system_private_key );
      ...
      const auto& accnt = control->db().get<account_object,by_name>( N(actc.wrap) );
      abi_def abi;
      BOOST_REQUIRE_EQUAL(abi_serializer::to_abi(accnt.abi, abi), true);
      abi_ser.set_abi(abi, abi_serializer_max_time);

      while( control->pending_block_producer().to_string() == "actc" ) {
         produce_block();
      }
   }
...
transaction actc_wrap_tester::wrap_exec( account_name executer, const transaction& trx, uint32_t expiration ) {
   fc::variants v;
...
transaction actc_wrap_tester::reqauth( account_name from, const vector<permission_level>& auths, uint32_t expiration ) {
   fc::variants v;
...
BOOST_AUTO_TEST_SUITE(actc_wrap_tests)

BOOST_FIXTURE_TEST_CASE( wrap_exec_direct, actc_wrap_tester ) try {
...

   {
      ...
      wrap_trx.actions.emplace_back( get_action( N(actc.wrap), N(exec),
                                                 {{N(alice), config::active_name}, {N(actc.wrap), config::active_name}},
                                                 mvo()
                                                   ("executer", "alice")
                                                   ("trx", trx)
      ) );
...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig, actc_wrap_tester ) try {
   auto trx = reqauth( N(bob), {permission_level{N(bob), config::active_name}} );
   ...
   BOOST_REQUIRE_EQUAL( "actc.wrap", name{traces[0]->action_traces[0].act.account} );
...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig_unapprove, actc_wrap_tester ) try {
   auto trx = reqauth( N(bob), {permission_level{N(bob), config::active_name}} );
   auto wrap_trx = wrap_exec( N(alice), trx );
   ...
   // The proposal should not have sufficient approvals to pass the authorization checks of actc.wrap::exec.
...
BOOST_FIXTURE_TEST_CASE( wrap_with_msig_producers_change, actc_wrap_tester ) try {
   ...
   // The proposal has four of the five requested approvals but they are not sufficient to satisfy the authorization checks of actc.wrap::exec.
   BOOST_REQUIRE_EXCEPTION( push_action( N(actc.msig), N(exec), N(alice), mvo()
                                          ("proposer",      "carol")
                                          ("proposal_name", "first")
                                          ("executer",      "alice")
