...
## 1. Installing the actc.wrap contract

The actc.wrap contract needs to be installed on a privileged account to function. It is recommended to use the account `actc.wrap`.

First, the account `actc.wrap` needs to be created. Since it has the restricted `actc.` prefix, only a privileged account can create this account. So this guide will use the `actc` account to create the `actc.wrap` account. On typical live blockchain configurations, the `actc` account can only be controlled by a supermajority of the current active block producers. So, this guide will use the `actc.msig` contract to help coordinate the approvals of the proposed transaction that creates the `actc.wrap` account.

The `actc.wrap` account also needs to have sufficient RAM to host the contract and sufficient CPU and network bandwidth to deploy the contract. This means that the creator of the account (`actc`) needs to gift sufficient RAM to the new account and delegate (preferably with transfer) sufficient bandwidth to the new account. To pull this off the `actc` account needs to have enough of the core system token (the `ACI` token will be used within this guide) in its liquid balance. So prior to continuing with the next steps of this guide, the active block producers of the chain who are coordinating this process need to ensure that a sufficient amount of core system tokens that they are authorized to spend is placed in the liquid balance of the `actc` account.

This guide will be using clactc to carry out the process.

### 1.1 Create the actc.wrap account

#### 1.1.1 Generate the transaction to create the actc.wrap account

The transaction to create the `actc.wrap` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the clactc command to propose the transaction to the actc.msig contract.
...
Fourth, generate a transaction containing the `actc::setpriv` action which will make the `actc.wrap` account privileged:
```
$ clactc push action -s -j -d actc setpriv '{"account": "actc.wrap", "is_priv": 1}' -p actc > generated_setpriv_trx.json
...
#### 1.1.2 Propose the transaction to create the actc.wrap account
...
#### 1.1.3 Review and approve the transaction to create the actc.wrap account
...
#### 1.1.4 Execute the transaction to create the actc.wrap account
...
Anyone can now verify that the `actc.wrap` was created:
```
$ clactc get account actc.wrap
privileged: true
permissions:
...
### 1.2 Deploy the actc.wrap contract

#### 1.2.1  Generate the transaction to deploy the actc.wrap contract

The transaction to deploy the contract to the `actc.wrap` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the clactc command to propose the transaction to the actc.msig contract.

The easy way to generate this transaction is using clactc:
```
$ clactc set contract -s -j -d actc.wrap contracts/actc.wrap/ > deploy_wrap_contract_trx.json
Reading WAST/WASM from contracts/actc.wrap/actc.wrap.wasm...
...
#### 1.2.2 Propose the transaction to deploy the actc.wrap contract
...
#### 1.2.3 Review and approve the transaction to deploy the actc.wrap contract
...
#### 1.2.4 Execute the transaction to create the actc.wrap account
...
Anyone can now verify that the `actc.wrap` contract was deployed correctly.

```
$ clactc get code -a retrieved-actc.wrap.abi actc.wrap
code hash: 1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c
saving abi to retrieved-actc.wrap.abi
$ sha256sum contracts/actc.wrap/actc.wrap.wasm
1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c  contracts/actc.wrap/actc.wrap.wasm
```

If the two hashes match then the local WebAssembly code is the one deployed on the blockchain. The retrieved ABI, which was stored in the file retrieved-actc.wrap.abi, can then be compared to the original ABI of the contract (contracts/actc.wrap/actc.wrap.abi) to ensure they are semantically the same.

## 2. Using the actc.wrap contract

### 2.1 Example: Updating owner authority of an arbitrary account

This example will demonstrate how to use the deployed actc.wrap contract together with the actc.msig contract to allow a greater than two-thirds supermajority of block producers of an ACTC blockchain to change the owner authority of an arbitrary account. The example will use clactc: in particular, the `clactc multisig` command, the `clactc set account permission` sub-command, and the `clactc wrap exec` sub-command. However, the guide also demonstrates what to do if the `clactc wrap exec` sub-command is not available.
...
The next step is to generate the transaction containing the `actc.wrap::exec` action. This action will contain the transaction in update_alice_owner_trx.json as part of its action payload data.
...
Then generate the template for the transaction containing the `actc.wrap::exec` action:
```
$ clactc push action -s -j -d actc.wrap exec '{"executer": "blkproducera", "trx": ""}' > wrap_update_alice_owner_trx.json
$ cat wrap_update_alice_owner_trx.json
{
