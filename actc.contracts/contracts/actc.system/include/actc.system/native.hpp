...
#include <actc/action.hpp>
#include <actc/contract.hpp>
#include <actc/crypto.hpp>
#include <actc/fixed_bytes.hpp>
#include <actc/ignore.hpp>
#include <actc/print.hpp>
#include <actc/privileged.hpp>
#include <actc/producer_schedule.hpp>

// This header is needed until `is_feature_activiated` and `preactivate_feature` are added to `actc.cdt`
#include <actc/../../capi/actc/crypto.h>
...
namespace actcsystem {
    ...
    */
   struct [[actc::table("abihash"), actc::contract("actc.system")]] abi_hash {
      name              owner;
      checksum256       hash;
   ...
   /**
    * The ACTC core native contract that governs authorization and contracts' abi.
    *
    * @details
    */
   class [[actc::contract("actc.system")]] native : public actc::contract {
      public:
          ...
          * This is useful because when doing authorization checks, the ACTC based blockchain starts with the
          * action needed to be authorized (and the contract belonging to), and looks up which permission
          * is needed to pass authorization validation. If a link is set, that permission is used for authoraization
          * validation otherwise then active is the default, with the exception of `actc.any`.
          * `actc.any` is an implicit permission which exists on every account; you can link actions to `actc.any`
         ...
         using newaccount_action = actc::action_wrapper<"newaccount"_n, &native::newaccount>;
         using updateauth_action = actc::action_wrapper<"updateauth"_n, &native::updateauth>;
         using deleteauth_action = actc::action_wrapper<"deleteauth"_n, &native::deleteauth>;
         using linkauth_action = actc::action_wrapper<"linkauth"_n, &native::linkauth>;
         using unlinkauth_action = actc::action_wrapper<"unlinkauth"_n, &native::unlinkauth>;
         using canceldelay_action = actc::action_wrapper<"canceldelay"_n, &native::canceldelay>;
         using setcode_action = actc::action_wrapper<"setcode"_n, &native::setcode>;
         using setabi_action = actc::action_wrapper<"setabi"_n, &native::setabi>;
   };
   /** @}*/ // @addtogroup actcsystem
}
