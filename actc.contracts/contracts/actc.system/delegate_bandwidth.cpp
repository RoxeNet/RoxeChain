...
#include <actc.token/actc.token.hpp>
...
namespace actcsystem {
    ...
    */
   typedef actc::multi_index< N(userres), user_resources>      user_resources_table;
   typedef actc::multi_index< N(delband), delegated_bandwidth> del_bandwidth_table;
   typedef actc::multi_index< N(refunds), refund_request>      refunds_table;
   ...
   void system_contract::buyrambytes( account_name payer, account_name receiver, uint32_t bytes ) {
      ...
      auto tmp = *itr;
      auto actcout = tmp.convert( asset(bytes,S(0,RAM)), CORE_SYMBOL );

      buyram( payer, receiver, actcout );
   }
   ...
   {
      require_auth( payer );
      actc_assert( quant.amount > 0, "must purchase a positive amount" );
      ...
      INLINE_ACTION_SENDER(actc::token, transfer)( N(actc.token), {payer,N(active)},
         { payer, N(actc.ram), quant_after_fee, std::string("buy ram") } );

      if( fee.amount > 0 ) {
         INLINE_ACTION_SENDER(actc::token, transfer)( N(actc.token), {payer,N(active)},
                                                       { payer, N(actc.ramfee), fee, std::string("ram fee") } );
      }
      ...
      actc_assert( bytes_out > 0, "must reserve a positive amount" );
   ...
   void system_contract::sellram( account_name account, int64_t bytes ) {
      ...
      actc_assert( bytes > 0, "cannot sell negative byte" );

      user_resources_table  userres( _self, account );
      auto res_itr = userres.find( account );
      actc_assert( res_itr != userres.end(), "no resource row" );
      actc_assert( res_itr->ram_bytes >= bytes, "insufficient quota" );
      ...
      actc_assert( tokens_out.amount > 1, "token amount received from selling ram is too low" );
      ...
      actc_assert( _gstate.total_ram_stake >= 0, "error, attempt to unstake more tokens than previously staked" );
      ...
      INLINE_ACTION_SENDER(actc::token, transfer)( N(actc.token), {N(actc.ram),N(active)},
                                                       { N(actc.ram), account, asset(tokens_out), std::string("sell ram") } );

      auto fee = ( tokens_out.amount + 199 ) / 200; /// .5% fee (round up)
      // since tokens_out.amount was asserted to be at least 2 earlier, fee.amount < tokens_out.amount
      
      if( fee > 0 ) {
         INLINE_ACTION_SENDER(actc::token, transfer)( N(actc.token), {account,N(active)},
            { account, N(actc.ramfee), asset(fee), std::string("sell ram fee") } );
      }
   }
   ...
   void validate_b1_vesting( int64_t stake ) {
      ...
      actc_assert( max_claimable - claimable <= stake, "b1 can only claim their tokens over 10 years" );
   }
   ...
   {
      require_auth( from );
      actc_assert( stake_net_delta != asset(0) || stake_cpu_delta != asset(0), "should stake non-zero amount" );
      actc_assert( std::abs( (stake_net_delta + stake_cpu_delta).amount )
      ...
      // update stake delegated from "from" to "receiver"
      {
         ...
         actc_assert( asset(0) <= itr->net_weight, "insufficient staked net bandwidth" );
         actc_assert( asset(0) <= itr->cpu_weight, "insufficient staked cpu bandwidth" );
      ...
      // update totals of "receiver"
      {
         ...
         actc_assert( asset(0) <= tot_itr->net_weight, "insufficient staked total net bandwidth" );
         actc_assert( asset(0) <= tot_itr->cpu_weight, "insufficient staked total cpu bandwidth" );
      ...
      if ( N(actc.stake) != source_stake_from ) { //for actc both transfer and refund make no sense
         ...
         if( is_delegating_to_self || is_undelegating ) {
            if ( req != refunds_tbl.end() ) { //need to update refund
               ...
               actc_assert( asset(0) <= req->net_amount, "negative net refund amount" ); //should never happen
               actc_assert( asset(0) <= req->cpu_amount, "negative cpu refund amount" ); //should never happen
         ...
         if ( asset(0) < transfer_amount ) {
            INLINE_ACTION_SENDER(actc::token, transfer)( N(actc.token), {source_stake_from, N(active)},
               { source_stake_from, N(actc.stake), asset(transfer_amount), std::string("stake bandwidth") } );
         }
      }
      ...
      // update voting power
      {
         ...
         actc_assert( 0 <= from_voter->staked, "stake for voting cannot be negative");
         if( from == N(b1) ) {
   ...
   {
      actc_assert( stake_cpu_quantity >= asset(0), "must stake a positive amount" );
      actc_assert( stake_net_quantity >= asset(0), "must stake a positive amount" );
      actc_assert( stake_net_quantity + stake_cpu_quantity > asset(0), "must stake a positive amount" );
      actc_assert( !transfer || from != receiver, "cannot use transfer flag if delegating to self" );
   ...
   {
      actc_assert( asset() <= unstake_cpu_quantity, "must unstake a positive amount" );
      actc_assert( asset() <= unstake_net_quantity, "must unstake a positive amount" );
      actc_assert( asset() < unstake_cpu_quantity + unstake_net_quantity, "must unstake a positive amount" );
      actc_assert( _gstate.total_activated_stake >= min_activated_stake,
   ...
   void system_contract::refund( const account_name owner ) {
      ...
      actc_assert( req != refunds_tbl.end(), "refund request not found" );
      actc_assert( req->request_time + refund_delay <= now(), "refund is not available yet" );
      // Until now() becomes NOW, the fact that now() is the timestamp of the previous block could in theory
      // allow people to get their tokens earlier than the 3 day delay if the unstake happened immediately after many
      // consecutive missed blocks.

      INLINE_ACTION_SENDER(actc::token, transfer)( N(actc.token), {N(actc.stake),N(active)},
                                                    { N(actc.stake), req->owner, req->net_amount + req->cpu_amount, std::string("unstake") } );
