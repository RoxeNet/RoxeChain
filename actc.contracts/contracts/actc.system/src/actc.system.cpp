#include <actc.system/actc.system.hpp>
#include <actc.token/actc.token.hpp>

#include <actc/crypto.hpp>
#include <actc/dispatcher.hpp>

#include <cmath>

namespace actcsystem {

   using actc::current_time_point;
   using actc::token;
    ...
    _rexorders(get_self(), get_self().value)
   {
      ...
      _gstate2 = _global2.exists() ? _global2.get() : actc_global_state2{};
      _gstate3 = _global3.exists() ? _global3.get() : actc_global_state3{};
   ...
   actc_global_state4 system_contract::get_default_inflation_parameters() {
      actc_global_state4 gs4;
   ...
   void system_contract::update_ram_supply() {
      auto cbt = actc::current_block_time();
   ...
   void system_contract::activate( const actc::checksum256& feature_digest ) {
      require_auth( get_self() );
   ...
   void native::setabi( const name& acnt, const std::vector<char>& abi ) {
      actc::multi_index< "abihash"_n, abi_hash >  table(get_self(), get_self().value);
      auto itr = table.find( acnt.value );
      if( itr == table.end() ) {
         table.emplace( acnt, [&]( auto& row ) {
            row.owner = acnt;
            row.hash = actc::sha256(const_cast<char*>(abi.data()), abi.size());
         });
      } else {
   ...
   void system_contract::init( unsigned_int version, const symbol& core ) {
      ...
      auto system_token_supply   = actc::token::get_supply(token_account, core.code() );
