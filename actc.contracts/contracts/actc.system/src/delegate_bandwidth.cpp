...
// Unfortunately, this is needed until CDT fixes the duplicate symbol error with actc::send_deferred
...
namespace actcsystem {
   ...
   void system_contract::buyrambytes( const name& payer, const name& receiver, uint32_t bytes ) {
      ...
      const int64_t actc_reserve   = itr->quote.balance.amount;
      const int64_t cost          = exchange_state::get_bancor_input( ram_reserve, actc_reserve, bytes );
  ...
  /**
                                   ...
                                   const asset& stake_net_delta, const asset& stake_cpu_delta, bool transfer )
   {
      ...
      if ( stake_account != source_stake_from ) { //for actc both transfer and refund make no sense
         ...
         if ( need_deferred_trx ) {
            ...
            actc::cancel_deferred( from.value ); // TODO: Remove this line when replacing deferred trxs is fixed
            out.send( from.value, from, true );
         } else {
            actc::cancel_deferred( from.value );
         }
