...
namespace actcsystem {
   ...
   using actc::microseconds;
   using actc::singleton;

   void system_contract::regproducer( const name& producer, const actc::public_key& producer_key, const std::string& url, uint16_t location ) {
      check( url.size() < 512, "url too long" );
      check( producer_key != actc::public_key(), "public key should not be the default value" );
