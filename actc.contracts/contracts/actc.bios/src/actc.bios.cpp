...
void bios::setprods( std::vector<actc::producer_key> schedule ) {
   require_auth( get_self() );
   set_proposed_producers( schedule );
}

void bios::setparams( const actc::blockchain_parameters& params ) {
   require_auth( get_self() );
...
void bios::activate( const actc::checksum256& feature_digest ) {
   require_auth( get_self() );
   preactivate_feature( feature_digest );
}

void bios::reqactivated( const actc::checksum256& feature_digest ) {
