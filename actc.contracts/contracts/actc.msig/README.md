...
Create a proposal
## actc.msig::propose    proposer proposal_name requested trx
...
Approve a proposal
## actc.msig::approve    proposer proposal_name level
   - **proposer** account proposing a transaction
...
## actc.msig::unapprove    proposer proposal_name level
   - **proposer** account proposing a transaction
   - **proposal_name** name of the proposal
...
Cancel a proposal
## actc.msig::cancel    proposer proposal_name canceler
   - **proposer** account proposing a transaction
   - **proposal_name** name of the proposal
   - **canceler** account canceling the transaction (only proposer can cancel not expired transaction)

Execute a proposal
## actc.msig::exec    proposer proposal_name executer
   - **proposer** account proposing a transaction
   - **proposal_name** name of the proposal
   - **executer** account executing the transaction


Clactc usage example.

Prerequisites:
   - actc.token contract installed to actc.token account, actc.msig contract installed on actc.msig account which is a privileged account.
   - account 'treasury' is the issuer of actc token.
...
````
$ clactc multisig propose test '[{"actor": "treasury", "permission": "active"}]' '[{"actor": "treasury", "permission": "active"}]' actc.token issue '{"to": "tester", "quantity": "1000.0000 actc", "memo": ""}' -p tester
...
````
$ clactc multisig review tester test -p treasury
{
  "proposal_name": "test",
  "requested_approvals": [{
      "actor": "treasury",
