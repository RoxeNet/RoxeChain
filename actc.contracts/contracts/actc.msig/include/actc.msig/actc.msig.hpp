   ...
   class [[actc::contract("actc.msig")]] multisig : public contract {
      public:
         ...
         void approve( name proposer, name proposal_name, permission_level level,
                       const actc::binary_extension<actc::checksum256>& proposal_hash );
         /**
          * Revoke proposal
          *
         ...
         using propose_action = actc::action_wrapper<"propose"_n, &multisig::propose>;
         using approve_action = actc::action_wrapper<"approve"_n, &multisig::approve>;
         using unapprove_action = actc::action_wrapper<"unapprove"_n, &multisig::unapprove>;
         using cancel_action = actc::action_wrapper<"cancel"_n, &multisig::cancel>;
         using exec_action = actc::action_wrapper<"exec"_n, &multisig::exec>;
         using invalidate_action = actc::action_wrapper<"invalidate"_n, &multisig::invalidate>;

      private:
         struct [[actc::table]] proposal {
            name                            proposal_name;
         ...
         typedef actc::multi_index< "proposal"_n, proposal > proposals;

         struct [[actc::table]] old_approvals_info {
            name                            proposal_name;
         ...
         typedef actc::multi_index< "approvals"_n, old_approvals_info > old_approvals;
         ...
         struct [[actc::table]] approvals_info {
            uint8_t                 version = 1;
            name                    proposal_name;
         ...
         typedef actc::multi_index< "approvals2"_n, approvals_info > approvals;

         struct [[actc::table]] invalidation {
            name         account;
