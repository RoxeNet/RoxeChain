...
class roxe_token_tester : public tester {
public:

   roxe_token_tester() {
      produce_blocks( 2 );

      create_accounts( { N(alice), N(bob), N(carol), N(roxe.token) } );
      produce_blocks( 2 );
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      auto symb = roxe::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(roxe.token), acc, N(accounts), symbol_code );
   ...
   action_result create( account_name issuer,
      ...
      return push_action( N(roxe.token), N(create), mvo()
           ( "issuer", issuer)
...
BOOST_AUTO_TEST_SUITE(roxe_token_tests)

BOOST_FIXTURE_TEST_CASE( create_tests, roxe_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_negative_max_supply, roxe_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( symbol_already_exists, roxe_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_supply, roxe_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_decimals, roxe_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( issue_tests, roxe_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( retire_tests, roxe_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( transfer_tests, roxe_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( open_tests, roxe_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( close_tests, roxe_token_tester ) try {
