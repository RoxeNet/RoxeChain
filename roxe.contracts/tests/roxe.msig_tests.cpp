...
class roxe_msig_tester : public tester {
public:
   roxe_msig_tester() {
      create_accounts( { N(roxe.msig), N(roxe.stake), N(roxe.ram), N(roxe.ramfee), N(alice), N(bob), N(carol) } );
      produce_block();
      ...
      set_code( N(roxe.msig), contracts::msig_wasm() );
      set_abi( N(roxe.msig), contracts::msig_abi().data() );

      produce_blocks();
      const auto& accnt = control->db().get<account_object,by_name>( N(roxe.msig) );
      abi_def abi;
   ...
   transaction_trace_ptr create_account_with_resources( account_name a, account_name creator, asset ramfunds, bool multisig,
      ...
      trx.actions.emplace_back( get_action( N(roxe), N(buyram), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("payer", creator)
                                            ("receiver", a)
                                            ("quant", ramfunds) )
      ...
      trx.actions.emplace_back( get_action( N(roxe), N(delegatebw), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("from", creator)
                                            ("receiver", a)
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(roxe.token), N(issue), manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const string& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(roxe.token), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
   ...
   asset get_balance( const account_name& act ) {
      ...
      const auto* tbl = db.find<table_id_object, by_code_scope_table>(boost::make_tuple(N(roxe.token), act, N(accounts)));
      share_type result = 0;
   ...
   transaction_trace_ptr push_action( const account_name& signer, const action_name& name, const variant_object& data, bool auth = true ) {
      ...
      auto trace = base_tester::push_action( N(roxe.msig), name, accounts, data );
      produce_block();
...
transaction roxe_msig_tester::reqauth( account_name from, const vector<permission_level>& auths, const fc::microseconds& max_serialization_time ) {
   fc::variants v;
...
BOOST_AUTO_TEST_SUITE(roxe_msig_tests)

BOOST_FIXTURE_TEST_CASE( propose_approve_execute, roxe_msig_tester ) try {
   ...
   BOOST_REQUIRE_EXCEPTION( push_action( N(alice), N(exec), mvo()
                            ...
                            roxe_assert_message_is("transaction authorization failed")
   );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_unapprove, roxe_msig_tester ) try {
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_by_two, roxe_msig_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( propose_with_wrong_requested_auth, roxe_msig_tester ) try {
   auto trx = reqauth("alice", vector<permission_level>{ { N(alice), config::active_name },  { N(bob), config::active_name } }, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( big_transaction, roxe_msig_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, roxe_msig_tester ) try {

   // required to set up the link between (roxe active) and (roxe.prods active)
   //
   //                  roxe active
   //                       |
   //             roxe.prods active (2/3 threshold)
   //             /         |        \             <--- implicitly updated in onblock action
   // alice active     bob active   carol active

   set_authority(N(roxe), "active", authority(1,
      vector<key_weight>{{get_private_key("roxe", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{N(roxe.prods), config::active_name}, 1}}), "owner",
      { { N(roxe), "active" } }, { get_private_key( N(roxe), "active" ) });

   set_producers( {N(alice),N(bob),N(carol)} );
   produce_blocks(50);

   create_accounts( { N(roxe.token), N(roxe.rex) } );
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"),
                        get_balance("roxe") + get_balance("roxe.ramfee") + get_balance("roxe.stake") + get_balance("roxe.ram") );
   ...
   vector<permission_level> action_perm = {{N(roxe), config::active_name}};
   ...
   // execute by alice to replace the roxe system contract
   transaction_trace_ptr trace;
   control->applied_transaction.connect(
   [&]( std::tuple<const transaction_trace_ptr&, const signed_transaction&> p ) {
      const auto& t = std::get<0>(p);
   ...
   BOOST_REQUIRE_EXCEPTION( create_account_with_resources( N(alice1111112), N(roxe), core_sym::from_string("1.0000"), false ),
                            roxe_assert_message_exception, roxe_assert_message_is("rejecting all actions")

   );
} FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE( update_system_contract_major_approve, roxe_msig_tester ) try {

   // set up the link between (roxe active) and (roxe.prods active)
   set_authority(N(roxe), "active", authority(1,
      vector<key_weight>{{get_private_key("roxe", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{N(roxe.prods), config::active_name}, 1}}), "owner",
      { { N(roxe), "active" } }, { get_private_key( N(roxe), "active" ) });
   ...
   create_currency( N(roxe.token), config::system_account_name, core_sym::from_string("10000000000.0000") );
   issue(config::system_account_name, core_sym::from_string("1000000000.0000"));
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"), get_balance( "roxe" ) );
   ...
   BOOST_REQUIRE_EXCEPTION(
      ...
      roxe_assert_message_exception, roxe_assert_message_is("transaction authorization failed")
   );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_invalidate, roxe_msig_tester ) try {
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( propose_invalidate_approve, roxe_msig_tester ) try {
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( approve_execute_old, roxe_msig_tester ) try {
   set_code( N(roxe.msig), contracts::util::msig_wasm_old() );
   set_abi( N(roxe.msig), contracts::util::msig_abi_old().data() );
   produce_blocks();

   //propose with old version of roxe.msig
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
   push_action( N(alice), N(propose), mvo()
                  ("proposer",      "alice")
                  ("proposal_name", "first")
...
BOOST_FIXTURE_TEST_CASE( approve_unapprove_old, roxe_msig_tester ) try {
   set_code( N(roxe.msig), contracts::util::msig_wasm_old() );
   set_abi( N(roxe.msig), contracts::util::msig_abi_old().data() );
   produce_blocks();
...
BOOST_FIXTURE_TEST_CASE( approve_by_two_old, roxe_msig_tester ) try {
   set_code( N(roxe.msig), contracts::util::msig_wasm_old() );
   set_abi( N(roxe.msig), contracts::util::msig_abi_old().data() );
   produce_blocks();
...
BOOST_FIXTURE_TEST_CASE( approve_with_hash, roxe_msig_tester ) try {
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
   ...
   BOOST_REQUIRE_EXCEPTION( push_action( N(alice), N(approve), mvo()
                            ...
                            roxe::chain::crypto_api_exception,
...
BOOST_FIXTURE_TEST_CASE( switch_proposal_and_fail_approve_with_hash, roxe_msig_tester ) try {
