...
namespace roxe_system {

class roxe_system_tester : public TESTER {
public:

   void basic_setup() {
      produce_blocks( 2 );

      create_accounts({ N(roxe.token), N(roxe.ram), N(roxe.ramfee), N(roxe.stake),
               N(roxe.bpay), N(roxe.vpay), N(roxe.saving), N(roxe.names), N(roxe.rex) });
      ...
      {
         const auto& accnt = control->db().get<account_object,by_name>( N(roxe.token) );
         abi_def abi;
   ...
   void create_core_token( symbol core_symbol = symbol{CORE_SYM} ) {
      ...
      create_currency( N(roxe.token), config::system_account_name, asset(100000000000000, core_symbol) );
      issue( asset(10000000000000, core_symbol) );
      BOOST_REQUIRE_EQUAL( asset(10000000000000, core_symbol), get_balance( "roxe", core_symbol ) );
   }
   ...
   void remaining_setup() {
      ...
      BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"), get_balance("roxe")  + get_balance("roxe.ramfee") + get_balance("roxe.stake") + get_balance("roxe.ram") );
   }
   ...
   roxe_system_tester( setup_level l = setup_level::full ) {
   ...
   roxe_system_tester(Lambda setup) {
      setup(*this);
   ...
   action_result buyram( const account_name& payer, account_name receiver, const asset& roxein ) {
      return push_action( payer, N(buyram), mvo()( "payer",payer)("receiver",receiver)("quant",roxein) );
   }
   ...
   fc::variant get_last_loan(bool cpu) {
      ...
      namespace chain = roxe::chain;
      auto table = cpu ? N(cpuloan) : N(netloan);
      const auto* t_id = db.find<roxe::chain::table_id_object, chain::by_code_scope_table>( boost::make_tuple( config::system_account_name, config::system_account_name, table ) );
      if ( !t_id ) {
         return fc::variant();
      }
   ...
   fc::variant get_rex_pool() const {
      ...
      const auto& db = control->db();
      namespace chain = roxe::chain;
      const auto* t_id = db.find<roxe::chain::table_id_object, chain::by_code_scope_table>( boost::make_tuple( config::system_account_name, config::system_account_name, N(rexpool) ) );
      if ( !t_id ) {
         return fc::variant();
      }
   ...
   fc::variant get_rex_return_pool() const {
      ...
      const auto& db = control->db();
      namespace chain = roxe::chain;
      const auto* t_id = db.find<roxe::chain::table_id_object, chain::by_code_scope_table>( boost::make_tuple( config::system_account_name, config::system_account_name, N(rexretpool) ) );
      if ( !t_id ) {
         return fc::variant();
      }
   ...
   fc::variant get_rex_return_buckets() const {
      ...
      const auto& db = control->db();
      namespace chain = roxe::chain;
      const auto* t_id = db.find<roxe::chain::table_id_object, chain::by_code_scope_table>( boost::make_tuple( config::system_account_name, config::system_account_name, N(retbuckets) ) );
      if ( !t_id ) {
         return fc::variant();
      }
...
//   }
   ...
   asset get_balance( const account_name& act, symbol balance_symbol = symbol{CORE_SYM} ) {
      vector<char> data = get_row_by_account( N(roxe.token), act, N(accounts), balance_symbol.to_symbol_code().value );
   ...
   void issue_and_transfer( const name& to, const asset& amount, const name& manager = config::system_account_name ) {
      signed_transaction trx;
      trx.actions.emplace_back( get_action( N(roxe.token), N(issue),
      ...
      if ( to != manager ) {
         trx.actions.emplace_back( get_action( N(roxe.token), N(transfer),
                                               vector<permission_level>{{manager, config::active_name}},
                                               mutable_variant_object()
                                               ("from",     manager)
                                               ("to",       to )
   ...
   fc::variant get_stats( const string& symbolname ) {
      ...
      vector<char> data = get_row_by_account( N(roxe.token), symbol_code, N(stat), symbol_code );
   ...
   fc::variant get_global_state2() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "roxe_global_state2", data, abi_serializer_max_time );
   }
   ...
   fc::variant get_global_state3() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "roxe_global_state3", data, abi_serializer_max_time );
   }
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(roxe.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( "roxe", "roxe.msig", core_sym::from_string("5000.0000") ) );
         produce_block();
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( "roxe", "alice1111111", core_sym::from_string("650000000.0000"), "roxe" );
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::roxe_code_name}, 1},
   ...
   action_result setinflation( int64_t annual_rate, int64_t inflation_pay_factor, int64_t votepay_factor ) {
      return push_action( N(roxe), N(setinflation), mvo()
...
inline uint64_t M( const string& roxe_str ) {
   return core_sym::from_string( roxe_str ).get_amount();
}
