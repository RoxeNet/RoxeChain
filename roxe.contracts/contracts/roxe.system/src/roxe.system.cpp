...
namespace roxesystem {
    ...
    _rexorders(get_self(), get_self().value)
   {
      ...
      _gstate2 = _global2.exists() ? _global2.get() : roxe_global_state2{};
      _gstate3 = _global3.exists() ? _global3.get() : roxe_global_state3{};
      _gstate4 = _global4.exists() ? _global4.get() : get_default_inflation_parameters();
   }

   roxe_global_state system_contract::get_default_parameters() {
      roxe_global_state dp;
      get_blockchain_parameters(dp);
      return dp;
   }

   roxe_global_state4 system_contract::get_default_inflation_parameters() {
      roxe_global_state4 gs4;
   ...
   void system_contract::update_ram_supply() {
      auto cbt = roxe::current_block_time();
   ...
   void system_contract::setparams( const roxe::blockchain_parameters& params ) {
      require_auth( get_self() );
      (roxe::blockchain_parameters&)(_gstate) = params;
   ...
   void system_contract::activate( const roxe::checksum256& feature_digest ) {
      require_auth( get_self() );
   ...
   void native::setabi( const name& acnt, const std::vector<char>& abi ) {
      roxe::multi_index< "abihash"_n, abi_hash >  table(get_self(), get_self().value);
      auto itr = table.find( acnt.value );
      if( itr == table.end() ) {
         table.emplace( acnt, [&]( auto& row ) {
            row.owner = acnt;
            row.hash = roxe::sha256(const_cast<char*>(abi.data()), abi.size());
         });
      } else {
