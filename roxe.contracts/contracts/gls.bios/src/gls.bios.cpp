#include <roxe.bios/roxe.bios.hpp>
...
void bios::setprods( std::vector<roxe::producer_key> schedule ) {
   require_auth( get_self() );
   set_proposed_producers( schedule );
}

void bios::setparams( const roxe::blockchain_parameters& params ) {
   require_auth( get_self() );
...
void bios::activate( const roxe::checksum256& feature_digest ) {
   require_auth( get_self() );
   preactivate_feature( feature_digest );
}

void bios::reqactivated( const roxe::checksum256& feature_digest ) {
