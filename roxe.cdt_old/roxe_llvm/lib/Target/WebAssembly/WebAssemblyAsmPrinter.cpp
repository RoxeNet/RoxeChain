...
void WebAssemblyAsmPrinter::EmitEndOfAsmFile(Module &M) {
  ...
  bool has_abi    = false;
  bool has_roxe_action = false;
  bool has_roxe_notify = false;
  for (const auto &F : M) {
     if (F.hasFnAttribute("roxe_wasm_import"))
        has_import = true;
     if (F.hasFnAttribute("roxe_wasm_abi"))
        has_abi = true;
     if (F.hasFnAttribute("roxe_wasm_action"))
        has_roxe_action = true;
     if (F.hasFnAttribute("roxe_wasm_notify"))
        has_roxe_notify = true;
  }
  ...
  if (has_abi) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".roxe_abi";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("roxe_wasm_abi")) {
           StringRef abi = F.getFnAttribute("roxe_wasm_abi").getValueAsString();
  ...
  if (has_roxe_action) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".roxe_actions";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("roxe_wasm_action")) {
           StringRef action_name = F.getFnAttribute("roxe_wasm_action").getValueAsString();
  ...
  if (has_roxe_notify) {
     OutStreamer->PushSection(); 
     std::string SectionName = ".roxe_notify";
     MCSectionWasm *mySection =
         OutContext.getWasmSection(SectionName, SectionKind::getMetadata());
     OutStreamer->SwitchSection(mySection);
     for (const auto &F : M) {
        if (F.hasFnAttribute("roxe_wasm_notify")) {
           StringRef name = F.getFnAttribute("roxe_wasm_notify").getValueAsString();
