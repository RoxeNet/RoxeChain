...
namespace roxe { namespace cdt {
   struct abigen_exception : public std::exception {
      virtual const char* what() const throw() {
         return "roxe.abigen fatal error";
      }
   };
   ...
   class abigen : public generation_utils {
      ...
      void add_action( const clang::CXXRecordDecl* decl ) {
         abi_action ret;
         auto action_name = decl->getRoxeActionAttr()->getName();
         ...
         if (action_name.empty()) {
            try {
               ...
               std::cout << "Error, name <" <<decl->getName().str() << "> is an invalid ROXE name.\n";
               throw;
            }
            ...
            try {
               ...
               std::cout << "Error, name <" << action_name.str() << "> is an invalid ROXE name.\n";
               throw;
            }
      ...
      void add_action( const clang::CXXMethodDecl* decl ) {
         ...
         if (action_name.empty()) {
            try {
               ...
               std::cout << "Error, name <" <<decl->getNameAsString() << "> is an invalid ROXE name.\n";
            }
      ...
      void add_table( const clang::CXXRecordDecl* decl ) {
         ...
         auto table_name = decl->getRoxeTableAttr()->getName();
         if (!table_name.empty()) {
            try {
      ...
      void add_table( uint64_t name, const clang::CXXRecordDecl* decl ) {
         if (!(decl->isRoxeTable() && abigen::is_roxe_contract(decl, get_contract_name())))
            return;
         abi_table t;
         t.type = decl->getNameAsString();
      ...
      std::string generate_json_comment() {
         std::stringstream ss;
         ss << "This file was generated with roxe-abigen.";
         ss << " DO NOT EDIT ";
         return ss.str(); 
      }
