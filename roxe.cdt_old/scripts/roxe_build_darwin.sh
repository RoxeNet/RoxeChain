	...
	if [ "${OS_MIN}" -lt 12 ]; then
		echo "You must be running Mac OS 10.12.x or higher to install ROXE."
		echo "Exiting now."
		exit 1
	fi

	if [ "${DISK_AVAIL}" -lt "$DISK_MIN" ]; then
		echo "You must have at least ${DISK_MIN}GB of available storage to install ROXE."
		echo "Exiting now."
		exit 1
	fi
	...
	then
		printf "\\tHomebrew must be installed to compile ROXEchain\\n\\n"
	...
	if [ $COUNT -gt 1 ]; then
		printf "\\n\\tThe following dependencies are required to install ROXE.\\n"
		printf "\\n\\t%s\\n\\n" "${DISPLAY}"
