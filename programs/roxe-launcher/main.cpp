...
using namespace roxe::launcher::config;
...
void
roxed_def::set_host( host_def* h, bool is_bios ) {
  host = h->host_name;
...
struct server_name_def {
  ...
  string roxe_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), roxe_home(), instances(1) {}
};
...
struct launcher_def {
   ...
   string roxed_extra_args;
   std::map<uint,string> specific_nodroxe_args;
   std::map<uint,string> specific_nodroxe_installation_paths;
   testnet_def network;
   string gelf_endpoint;
   ...
   roxe::chain::genesis_state genesis_from_file;

   void assign_name (roxed_def &node, bool is_bios);
   ...
   void prep_remote_config_dir (roxed_def &node, host_def *host);
   void launch (roxed_def &node, string &gts);
   void kill (launch_modes mode, string sig_opt);
   static string get_node_num(uint16_t node_num);
   pair<host_def, roxed_def> find_node(uint16_t node_num);
   vector<pair<host_def, roxed_def>> get_nodes(const string& node_number_list);
...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    (node_executable_name, bpo::value<string>(&roxed_extra_args), ("forward " + string(node_executable_name) + " command line argument(s) to each instance of " + string(node_executable_name) + ", enclose arg(s) in quotes").c_str())
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  retrieve_paired_array_parameters(vmap, "specific-num", "specific-" + string(node_executable_name), specific_nodroxe_args);
  retrieve_paired_array_parameters(vmap, "spcfc-inst-num", "spcfc-inst-" + string(node_executable_name), specific_nodroxe_installation_paths);
  ...
  if( !host_map_file.empty() ) {
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &roxed : binding.instances) {
          roxed.host = binding.host_name;
          roxed.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(roxed.p2p_port);

          aliases.push_back (roxed.name);
        }
      }
  ...
  if (vmap.count("specific-num")) {
    ...
    for(uint i = 0; i < specific_nums.size(); ++i)
    {
      ...
      specific_nodroxe_args[num] = specific_args[i];
    }
  }

  char* erd_env_var = getenv ("ROXE_HOME");
  ...
  stage = bfs::path(erd);
  if (!bfs::exists(stage)) {
    cerr << "\"" << erd << "\" is not a valid path. Please ensure environment variable ROXE_HOME is set to the build path." << endl;
    exit (-1);
  }
...
void
launcher_def::assign_name (roxed_def &node, bool is_bios) {
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    host_def local_host;
    local_host.roxe_home = erd;
    local_host.genesis = genesis.string();
    for (size_t i = 0; i < (total_nodes); i++) {
      roxed_def roxed;
      assign_name(roxed, i == 0);
      aliases.push_back(roxed.name);
      roxed.set_host (&local_host, i == 0);
      local_host.instances.emplace_back(move(roxed));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->roxe_home =
          (local_id.contains (lhost->host_name) || servers.default_roxe_home.empty()) ?
          erd : servers.default_roxe_home;
        host_ndx++;
      } // ph_count == 0

      roxed_def roxed;
      assign_name(roxed, do_bios);
      roxed.has_db = false;
      ...
      aliases.push_back(roxed.name);
      roxed.set_host (lhost, do_bios);
      do_bios = false;
      lhost->instances.emplace_back(move(roxed));
      --ph_count;
    } // for i
    bindings.emplace_back( move(*lhost) );
    delete lhost;
...
launcher_def::bind_nodes () {
   ...
   for (auto &h : bindings) {
      for (auto &inst : h.instances) {
         ...
         node.instance = &inst;
         auto kp = is_bios ?
            private_key_type(string("5KiNH96ufjdDuYsnY9HUNNJHGcX9cJRctyFQovv9Hwsnzodu7YU")) :
            private_key_type::generate();
         auto pubkey = kp.get_public_key();
         node.keys.emplace_back (move(kp));
         if (is_bios) {
            string prodname = "roxe";
...
launcher_def::deploy_config_files (tn_node_def &node) {
  boost::system::error_code ec;
  roxed_def &instance = *node.instance;
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->roxe_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->roxe_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->roxe_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->roxe_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->roxe_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (instance.has_db || node.producers.size()) {
    ...
    cfg << "plugin = roxe::producer_plugin\n";
  }
  if( instance.has_db ) {
    cfg << "plugin = roxe::mongo_db_plugin\n";
  }
  cfg << "plugin = roxe::net_plugin\n";
  cfg << "plugin = roxe::chain_api_plugin\n"
...
launcher_def::init_genesis () {
   ...
   if (!bfs::exists(genesis_path)) {
      ...
      roxe::chain::genesis_state default_genesis;
      fc::json::save_to_file( default_genesis, genesis_path, true );
   }
   string bioskey = string(network.nodes["bios"].keys[0].get_public_key());

   fc::json::from_file(genesis_path).as<roxe::chain::genesis_state>(genesis_from_file);
...
void
launcher_def::prep_remote_config_dir (roxed_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->roxe_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->roxe_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->roxe_home;

  cmd = "cd " + host->roxe_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->roxe_home
...
void
launcher_def::launch (roxed_def &instance, string &gts) {
  ...
  string install_path;
  if (instance.name != "bios" && !specific_nodroxe_installation_paths.empty()) {
     const auto node_num = boost::lexical_cast<uint16_t,string>(instance.get_node_num());
     if (specific_nodroxe_installation_paths.count(node_num)) {
        install_path = specific_nodroxe_installation_paths[node_num] + "/";
     }
  }
  string roxedcmd = install_path + "programs/nodroxe/" + string(node_executable_name) + " ";
  if (skip_transaction_signatures) {
    roxedcmd += "--skip-transaction-signatures ";
  }
  if (!roxed_extra_args.empty()) {
    if (instance.name == "bios") {
       ...
       regex r("--plugin +roxe::mongo_db_plugin");
       string args = std::regex_replace (roxed_extra_args,r,"");
       regex r2("--mongodb-uri +[^ ]+");
       args = std::regex_replace (args,r2,"");
       roxedcmd += args + " ";
    }
    else {
       roxedcmd += roxed_extra_args + " ";
    }
  }
  if (instance.name != "bios" && !specific_nodroxe_args.empty()) {
     const auto node_num = boost::lexical_cast<uint16_t,string>(instance.get_node_num());
     if (specific_nodroxe_args.count(node_num)) {
        roxedcmd += specific_nodroxe_args[node_num] + " ";
     }
  }

  if( add_enable_stale_production ) {
    roxedcmd += "--enable-stale-production true ";
    add_enable_stale_production = false;
  }

  roxedcmd += " --config-dir " + instance.config_dir_name + " --data-dir " + instance.data_dir_name;
  roxedcmd += " --genesis-json " + instance.config_dir_name + "/genesis.json";
  if (gts.length()) {
    roxedcmd += " --genesis-timestamp " + gts;
  }
  ...
  if (!host->is_local()) {
    ...
    string cmdl ("cd ");
    cmdl += host->roxe_home + "; nohup " + roxedcmd + " > "
    ...
    string cmd = "cd " + host->roxe_home + "; kill -15 $(cat " + pidf.string() + ")";
    format_ssh (cmd, host->host_name, info.kill_cmd);
  }
  else if (!instance.node->dont_start) {
    cerr << "spawning child, " << roxedcmd << endl;

    bp::child c(roxedcmd, bp::std_out > reout, bp::std_err > reerr );
    bfs::remove(reerr_sl);
    ...
    if(!c.running()) {
      cerr << "child not running after spawn " << roxedcmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
    ...
    cerr << "not spawning child, " << roxedcmd << endl;
...
#if 0
void
launcher_def::kill_instance(roxed_def, string sig_opt) {
}
#endif
...
vector<pair<host_def, roxed_def>>
launcher_def::get_nodes(const string& node_number_list) {
   vector<pair<host_def, roxed_def>> node_list;
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      const roxed_def& node = node_pair.second;
      const string node_num = node.get_node_num();
      cout << "Bouncing " << node.name << endl;
      string cmd = "./scripts/roxe-tn_bounce.sh " + roxed_extra_args;
      if (node_num != "bios" && !specific_nodroxe_args.empty()) {
         const auto node_num_i = boost::lexical_cast<uint16_t,string>(node_num);
         if (specific_nodroxe_args.count(node_num_i)) {
            cmd += " " + specific_nodroxe_args[node_num_i];
         }
      }

      do_command(host, node.name, { { "ROXE_HOME", host.roxe_home }, { "ROXE_NODE", node_num } }, cmd);
   }
}
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string cmd = "./scripts/roxe-tn_down.sh ";
      do_command(host, node.name,
                 { { "ROXE_HOME", host.roxe_home }, { "ROXE_NODE", node_num }, { "ROXE_TN_RESTART_CONFIG_DIR", node.config_dir_name } },
                 cmd);
   }
}
...
launcher_def::roll (const string& host_names) {
   ...
   for (string host_name: hosts) {
      ...
      string cmd = "./scripts/roxe-tn_roll.sh ";
      do_command(*host, host_name, { { "ROXE_HOME", host->roxe_home } }, cmd);
   }
}
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the roxe-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the roxe-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the roxe-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << roxe::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (roxe_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_roxe_home) (ssh) )
